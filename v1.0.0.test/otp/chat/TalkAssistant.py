...
from libotp import CFSpeech, CFTimeout, CFThought, CFExclaim
...
class TalkAssistant(DirectObject.DirectObject):
    ...
    def receiveWhisperTalk(self, avatarId, avatarName, accountId, accountName, toId, toName, message, scrubbed=0):
        ...
        if accountId:
            self.addToHistoryDISLId(newMessage, accountId)
        messenger.send('NewOpenMessage', [newMessage])
        self.logWhisperFrom(avatarName, message)
        return error
    ...
    def receiveExclaim(self, avatarId, avatarName, accountId, accountName, message, scrubbed=0):
        error = None
        if not avatarName and avatarId:
            avatarName = self.findAvatarName(avatarId)
        if not accountName and accountId:
            accountName = self.findPlayerName(accountId)
        newMessage = TalkMessage(self.countMessage(), self.stampTime(), message, avatarId, avatarName, accountId, accountName, None, None, None, None, AVATAR_EXCLAIM, None)
        if avatarId != localAvatar.doId:
            self.addHandle(avatarId, newMessage)
        reject = 0
        if avatarId:
    ...
    def sendWhisperTalk(self, message, receiverAvId):
        ...
        self.logWhisperTo(receiverAvId, message)
        return error
    ...
    def sendAvatarWhisperSpeedChat(self, type, messageIndex, receiverId):
        ...
        if self.logWhispers:
            ...
            messenger.send('NewOpenMessage', [newMessage])
        self.logWhisperTo(receiverId, message)
        return error
    ...
    def logWhisperTo(self, receiverAvId, chatString):
        receiver = base.cr.identifyAvatar(receiverAvId)
        if receiver and chatString.strip() and chatString != '.':
            fgColor, bgColor = NametagGlobals._whisper_colors[3][3]
            newBgColor = list(bgColor)
            newBgColor[3] = 1.0
            messenger.send('addChatHistory', [OTPLocalizer.MeToReceiver % receiver.getName().split('\n')[0], None, None, newBgColor, chatString.strip()])
        return

    def logWhisperFrom(self, receiverName, chatString):
        if chatString.strip() and chatString != '.':
            fgColor, bgColor = NametagGlobals._whisper_colors[3][3]
            newBgColor = list(bgColor)
            newBgColor[3] = 1.0
            messenger.send('addChatHistory', [OTPLocalizer.ReceiverToMe % receiverName.split('\n')[0], None, None, newBgColor, chatString.strip()])
        return

    def sendGlobalTalk(self, message):
        error = None
        magicWordPrefix = '~'
        magicWordActivatorIndex = base.settings.getInt('game', 'magic-word-activator', 0)
        MW_PREFIX_ALLOWED = ['~', '?', '/', '<', ':', ';']
        ...
        if base.cr.wantMagicWords and len(message) > 0 and message[0] == magicWordPrefix:
            ...
            base.cr.chatManager.sendGlobalMessage(message)
        return error