...
def getTransparentTexture():
    global TransparentTexture
    if TransparentTexture == None:
        image = PNMImage(1, 1)
        image.addAlpha()
        TransparentTexture = Texture()
        TransparentTexture.load(image)
...
AccessLevelName2Int = {'RESTRICTED': -100, 
   'NO_ACCESS': 0, 
   'USER': 100, 
   'MODERATOR': 200, 
   'ADMIN': 300, 
   ...
   'TTOFF_MODERATOR': 700, 
   'TTOFF_DEVELOPER': 800}
AccessLevelInt2Name = {-100: 'RESTRICTED', 
   0: 'NO_ACCESS', 
   100: 'USER', 
   200: 'MODERATOR', 
   300: 'ADMIN', 
...
BootedKeyboardChatAuth = 120
BootedConnectionKilled = 122
...
BootedNoReconnect = [BootedToonIssue, BootedBanned, BootedKicked]