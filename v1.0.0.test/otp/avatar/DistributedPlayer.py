...
from libotp import CFQuicktalker, CFPageButton, CFQuitButton, CFSpeech, CFThought, CFTimeout, CFExclaim
...
class DistributedPlayer(DistributedAvatar.DistributedAvatar, PlayerBase.PlayerBase, TelemetryLimited):
    ...
    def displayTalkWhisper(self, fromId, avatarName, chatString, mods, raw):
        print 'TalkWhisper from %s: %s' % (fromId, chatString)

    def scrubTalk(self, chat, mods, raw):
        return chat
    ...
    def setSC(self, msgIndex, displayType=0):
        ...
        if chatString:
            if displayType == 1:
                self.setChatAbsolute(chatString, CFExclaim | CFQuicktalker | CFTimeout, quiet=1)
            elif displayType == 2:
    ...
    def teleportQuery(self, requesterId):
        ...
        if avatar != None:
            ...
            teleportNotify.debug("couldn't find requester with id %s" % requesterId)
            base.cr.centralLogger.writeClientEvent('Hacker alert - someone tried to disconnect client with teleportQuery(%s)' % requesterId)
        return
    ...
    def setAccessLevel(self, accessLevel):
        ...
        if self.isLocal():
            self.cr.wantMagicWords = self.accessLevel >= OTPGlobals.AccessLevelName2Int.get('USER')
