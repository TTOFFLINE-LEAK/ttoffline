import sys, time, string, random, gc, os, uuid
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher=None, playGame=None):
        ...
        self.accept('showPasswordScreen', self.__handlePasswordScreen)
        return
    ...
    def readDCFile(self, dcFileNames=None):
        return DCReader.readDCFile(self, dcFileNames)
    ...
    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def __handlePasswordScreen(self):
    ...
    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def exitLogin(self):
        if not self.gameServicesManager.password:
            self.cleanupWaitingForDatabase()
            self.ignore(self.loginDoneEvent)
            del self.loginDoneEvent
            self.handler = None
    ...
    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def enterNoShards(self, status):
        messenger.send('connectionIssue')
        self.handler = self.handleMessageType
        dialogClass = OTPGlobals.getGlobalDialogClass()
        message = OTPLocalizer.CRNoDistrictsTryAgain
        if status == 'full':
            message = OTPLocalizer.CRFullDistrictsTryAgain
        self.noShardsBox = dialogClass(message=message, doneEvent='noShardsAck', style=OTPDialog.TwoChoice)
    ...
    def enterNoConnection(self):
        ...
        if self.bootedIndex is not None and self.bootedIndex in OTPLocalizer.CRBootedReasons.keys():
