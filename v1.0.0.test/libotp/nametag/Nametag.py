...
class Nametag(ClickablePopup):
    ...
    def determineContents(self):
        if self.m_group and self.m_group.isManaged():
            ...
            if v4 & CFSpeech:
                ...
                if v4 & CFThought and v3 & Nametag.CThought:
                    return Nametag.CThought
                if v4 & CFExclaim and v3 & Nametag.CSpeech:
                    return Nametag.CExclaim
    ...
    def startFlash(self, np):
        self.stopFlash()
        self.m_ival = Sequence(np.colorInterval(0.5, Vec4(1.0, 1.0, 1.0, 0.5), startColor=Vec4(1.0, 1.0, 1.0, 1.0), blendType='easeOut'), np.colorInterval(0.5, Vec4(1.0, 1.0, 1.0, 1.0), startColor=Vec4(1.0, 1.0, 1.0, 0.5), blendType='easeIn'))
        self.m_ival.loop()
