...
class NametagGroup:
    ...
    def __init__(self):
        ...
        self.m_manager = None
        self.m_region_seq += 1
        self.m_contents = CFSpeech | CFThought | CFQuicktalker | CFExclaim
        self.m_is_active = 1
    ...
    def getNameWordwrap(self):
        ...
        return {self.CCNoChat: 7.8, self.CCToonBuilding: 8.5, 
    ...
    def setPageNumber(self, page, force=False):
        if self.m_page_number == page and not force:
            return
        self.m_page_number = page
        if self.willHaveButton():
            self.m_timeout_start = globalClock.getFrameTime() + 0.2
            self.m_has_timeout = True
        chatString = self.getChat()
        if chatString.strip() and chatString != '.':
            if hasattr(base, 'localAvatar'):
                if self == base.localAvatar.nametag:
                    color = 8
                else:
                    color = self.getColorCode()
            else:
                color = self.getColorCode()
            messenger.send('addChatHistory', [self.getName().split('\n')[0], self.getNameFont(), self.getChatFont(), color, chatString.strip()])
        self.updateContentsAll()
