...
class SellbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def __init__(self, hood, parentFSMState, doneEvent):
        ...
        self.fsm.addState(State.State('factoryInterior', self.enterFactoryInterior, self.exitFactoryInterior, ['quietZone', 'start', 'factoryExterior']))
    ...
    def loadPlaceGeom(self, zoneId):
        ...
        if zoneId == ToontownGlobals.SellbotHQ:
            ...
            factoryTypeText = DirectGui.OnscreenText(text=TTLocalizer.Sellbot, font=ToontownGlobals.getSuitFont(), pos=(0,
                                                                                                                        -0.25), scale=0.075, mayChange=False, parent=factorySign)
            factoryTypeText.setDepthWrite(0)
            factoryText = DirectGui.OnscreenText(text=TTLocalizer.Factory, font=ToontownGlobals.getSuitFont(), pos=(0,
                                                                                                                    -0.34), scale=0.12, mayChange=False, parent=factorySign)
