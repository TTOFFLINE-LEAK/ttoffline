...
import DistributedBuildingAI, HQBuildingAI, GagshopBuildingAI, PetshopBuildingAI, LighthouseBuildingAI, TutorialBuildingAI
...
class DistributedBuildingMgrAI:
    ...
    def getToonBlocks(self):
        ...
        for i in self.__buildings.values():
            if isinstance(i, HQBuildingAI.HQBuildingAI):
                continue
            if isinstance(i, LighthouseBuildingAI.LighthouseBuildingAI):
                continue
            if isinstance(i, TutorialBuildingAI.TutorialBuildingAI):
                continue
    ...
    def getDNABlockLists(self):
        ...
        for i in xrange(self.dnaStore.getNumBlockNumbers()):
            ...
            elif buildingType == 'lighthouse':
                lighthouseBlocks.append(blockNumber)
            elif buildingType == 'tutorial':
                tutorialBlocks.append(blockNumber)
            else:
                blocks.append(blockNumber)

        return (
         blocks, hqBlocks, gagshopBlocks, petshopBlocks, kartshopBlocks, animBldgBlocks, lighthouseBlocks, tutorialBlocks)

    def findAllLandmarkBuildings(self):
        buildings = self.load()
        blocks, hqBlocks, gagshopBlocks, petshopBlocks, kartshopBlocks, animBldgBlocks, lighthouseBlocks, tutorialBlocks = self.getDNABlockLists()
        for block in blocks:
        ...
        for block in lighthouseBlocks:
            self.newLighthouseBuilding(block)

        for block in tutorialBlocks:
            self.newTutorialBuilding(block)
        ...
        if blockData:
            ...
            if blockData['state'] == 'suit':
                ...
                if simbase.air.wantCogdominiums:
                    building.numFloors = DistributedBuildingAI.DistributedBuildingAI.FieldOfficeNumFloors + (1 if blockData.get('track', 'c') in ('s',
                                                                                                                                                  'l') else 0)
    ...
    def newLighthouseBuilding(self, blockNumber):
        dnaStore = self.air.dnaStoreMap[self.canonicalBranchID]
        exteriorZoneId = dnaStore.getZoneFromBlockNumber(blockNumber)
        exteriorZoneId = ZoneUtil.getTrueZoneId(exteriorZoneId, self.branchID)
        interiorZoneId = self.branchID - self.branchID % 100 + 500 + blockNumber
        building = LighthouseBuildingAI.LighthouseBuildingAI(self.air, exteriorZoneId, interiorZoneId, blockNumber)
        self.__buildings[blockNumber] = building
        return building

    def newTutorialBuilding(self, blockNumber):
        dnaStore = self.air.dnaStoreMap[self.canonicalBranchID]
        exteriorZoneId = dnaStore.getZoneFromBlockNumber(blockNumber)
        exteriorZoneId = ZoneUtil.getTrueZoneId(exteriorZoneId, self.branchID)
        interiorZoneId = self.branchID - self.branchID % 100 + 500 + blockNumber
        building = TutorialBuildingAI.TutorialBuildingAI(self.air, exteriorZoneId, interiorZoneId, blockNumber)
        self.__buildings[blockNumber] = building
        return building
    ...
    def saveTo(self, file):
        ...
        for i in self.__buildings.values():
            ...
            blocks[str(jsonData['block']).encode('utf-8').strip()] = jsonData

        print ''
        print 'BLDGMGR BLOCKS'
        print blocks
        print 'END OF BLDGMRGR BLOCKS'
        print ''
