...
ElevatorData = {ELEVATOR_NORMAL: {'openTime': 2.0, 'closeTime': 2.0, 
   ...
   ELEVATOR_COUNTRY_CLUB: {'openTime': 2.0, 'closeTime': 2.0, 
                           ...
                           'sfxVolume': 1.0, 
                           'collRadius': 4}}
ELEVATOR_COUNTDOWNS = {ELEVATOR_NORMAL: 15.0, 
   ELEVATOR_VP: 30.0, 
   ELEVATOR_MINT: 15.0, 
   ELEVATOR_OFFICE: 15.0, 
   ELEVATOR_CFO: 30.0, 
   ELEVATOR_CJ: 30.0, 
   ELEVATOR_STAGE: 42.0, 
   ELEVATOR_BB: 30.0, 
   ELEVATOR_COUNTRY_CLUB: 15.0}

def getElevatorCountdown(type):
    if not config.GetBool('mini-server', False):
        return 2.0
    return ELEVATOR_COUNTDOWNS.get(type)
