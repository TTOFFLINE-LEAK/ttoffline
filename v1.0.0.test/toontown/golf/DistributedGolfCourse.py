...
class DistributedGolfCourse(DistributedObject.DistributedObject, FSM, DelayDeletable):
    ...
    def load(self):
        if self.courseId == 3:
            musPath = 'phase_12/audio/bgm/Bossbot_PlayGolf_v1'
        else:
            musPath = 'phase_6/audio/bgm/GZ_PlayGolf'
        self.music = base.loader.loadMusic(musPath + '.ogg')
    ...
    def enterCleanup(self):
        ...
        if title == TTLocalizer.GolfCourseNames.get(3):
            zone = ToontownGlobals.BossbotHQ
        base.cr.discordManager.setInfo(base.cr.discordManager.getState(), None, ToontownGlobals.Zone2String.get(zone), None, None, None, zone)
        return
