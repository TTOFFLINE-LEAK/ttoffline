...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def setupCamera(self, mode):
        ...
        if mode == PhoneGlobals.PHONE_MOVIE_PICKUP:
            quat = Quat()
            quat.setHpr((35, -8, 0))
            self.cameraLerp = LerpPosQuatInterval(camera, 1, (4, -4, base.localAvatar.getHeight() - 0.5), quat, blendType='easeOut', other=base.localAvatar, name=self.uniqueName('lerpCamera'))
