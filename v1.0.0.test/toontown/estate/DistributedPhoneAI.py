...
class LoadGiftAvatar:
    ...
    def __gotAvatar(self, dclass, fields):
        if dclass != self.air.dclassesByName['DistributedToonAI']:
            return
        for key in ('setDNAString', 'setMailboxContents', 'setAwardMailboxContents',
                    'setGiftSchedule', 'setDeliverySchedule', 'setAwardSchedule'):
            fields[key] = fields[key][0].encode('base64')

        newDict = self.copyDict(fields, 'setDNAString', 'setMailboxContents', 'setAwardMailboxContents', 'setGiftSchedule', 'setDeliverySchedule', 'setAwardSchedule', 'setHat', 'setGlasses', 'setBackpack', 'setShoes', 'setHatList', 'setGlassesList', 'setBackpackList', 'setShoesList', 'setCustomMessages', 'setEmoteAccess', 'setClothesTopsList', 'setClothesBottomsList', 'setPetTrickPhrases')
...
class DistributedPhoneAI(DistributedFurnitureItemAI):
    ...
    def __handleHouse(self, dclass, fields):
        ...
        numItems = len(interiorItems) + len(atticItems) + len(atticWallpaper) + len(atticWindows) + len(interiorWallpaper) + len(interiorWindows)
    ...
    def requestPurchaseMessage(self, context, blob, optional):
        ...
        av = self.air.doId2do.get(avId)
        if not av:
            self.air.writeServerEvent('suspicious', avId, 'av tried to purchase something but they are not on the district!')
            return
        if avId != self.avId:
    ...
    def requestGiftPurchaseMessage(self, context, targetId, blob, optional):
        ...
        av = self.air.doId2do.get(avId)
        if not av:
            self.air.writeServerEvent('suspicious', avId, 'av tried to gift something but they are not on the district!')
            return
        if avId != self.avId:
    ...
    def attemptGiftPurchase(self, avId, targetId, avatar, optional):
        ...
        if len(recipient.mailboxContents) >= ToontownGlobals.MaxMailboxContents:
            ...
            return
        if len(recipient.onOrder) + len(recipient.mailboxContents) + len(recipient.onGiftOrder) + 1 >= ToontownGlobals.MaxMailboxContents:
