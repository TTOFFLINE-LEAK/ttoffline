...
from toontown.hood.DistributedPropGeneratorAI import DistributedPropGeneratorAI
...
class DistributedHouseAI(DistributedObjectAI):
    ...
    def announceGenerate(self):
        ...
        self.propGenerator = DistributedPropGeneratorAI(self.air, self.interiorZone)
        self.propGenerator.generateWithRequired(self.interiorZone)
        self.air.propGenerators[self.interiorZone] = self.propGenerator
        if self.avatarId:
    ...
    def delete(self):
        ...
        if self.propGenerator:
            ...
            self.propGenerator = None
            if self.interiorZone in self.air.propGenerators:
                del self.air.propGenerators[self.interiorZone]
    ...
    def setInteriorWindows(self, interiorWindows):
        self.interiorWindows = CatalogItemList(interiorWindows, store=CatalogItem.Customization | CatalogItem.WindowPlacement)
