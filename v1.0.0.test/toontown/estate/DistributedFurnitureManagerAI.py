...
class DistributedFurnitureManagerAI(DistributedObjectAI):
    ...
    def recoverDeletedItemMessage(self, blob, index, context):
        ...
        type2attic = {CatalogSurfaceItem.CatalogSurfaceItem: (
    ...
    def loadFurniture(self):
        for item in CatalogItemList(self.house.getInteriorItems(), store=CatalogItem.Customization | CatalogItem.Location):
    ...
    def _verifyOwner(self, avId, message, context):
        if self.house.avatarId != avId:
            self.air.writeServerEvent('suspicious', avId, 'av tried to send perform furniture management operation %s while not owner!' % message)
    ...
    def getNumItems(self):
        numItems = len(self.house.interiorItems) + len(self.house.atticItems) + len(self.house.atticWallpaper) + len(self.house.atticWindows) + len(self.house.interiorWallpaper) + len(self.house.interiorWindows)
        return numItems
