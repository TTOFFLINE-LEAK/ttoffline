...
class TTOffMagicWordManager(DistributedObject.DistributedObject):
    ...
    def generateResponse(self, responseType, magicWord='', args=None, returnValue=None, affectRange=None, affectType=None, affectExtra=None, lastClickedAvId=None, extraMessageData=None):
        response = self.generateMagicWordResponse(responseType, magicWord, args, returnValue, affectRange, affectType, affectExtra, lastClickedAvId, extraMessageData)
        base.localAvatar.setSystemMessage(0, self.wizardName + ': ' + response, WhisperPopup.WTMagicWord)
        self.notify.info(response)

    def generateMagicWordResponse(self, responseType, magicWord, args, returnValue, affectRange, affectType, affectExtra, lastClickedAvId, extraMessageData):
        response = ''
    ...
    def executeMagicWord(self, word, commandName, targetIds, args, affectRange, affectType, affectExtra, lastClickedAvId):
        ...
        if returnValue:
            ...
            self.generateResponse(responseType='SuccessNoResp', magicWord=word, args=args, affectRange=affectRange, affectType=affectType, affectExtra=affectExtra, lastClickedAvId=lastClickedAvId)
        return