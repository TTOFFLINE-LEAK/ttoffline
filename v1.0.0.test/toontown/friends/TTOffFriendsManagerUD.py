...
class GetAvatarInfoOperation(FSM):
    ...
    def enterFinished(self):
        if not self.isPet:
            self.mgr.avBasicInfoCache[self.avId] = {'expire': time.time() + config.GetInt('friend-detail-cache-expire', 3600), 'avInfo': [
                        self.avId, self.fields['setName'][0], self.fields['setDNAString'][0],
...
class TTOffFriendsManagerUD(DistributedObjectGlobalUD):
    ...
    def __gotAvatarDetails(self, success, avId, fields, isPet):
        ...
        if not success:
            return
        if isPet:
            details = [['setTraitSeed', fields['setTraitSeed'][0]],
             [
            ...
            details = [['setName', fields['setName'][0]],
             [
