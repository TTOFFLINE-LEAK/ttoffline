...
class DistributedNPCTailor(DistributedNPCToonBase):
    ...
    def __handlePurchaseDone(self, timeout=0):
        if self.clothesGUI.doneStatus == 'last' or timeout == 1:
            ...
            which = 0
            if self.clothesGUI.topStyleChoice != -1 or self.clothesGUI.topColorChoice != -1:
                which = which | ClosetGlobals.SHIRT
            if self.clothesGUI.bottomStyleChoice != -1 or self.clothesGUI.bottomColorChoice != -1:
                which = which | ClosetGlobals.SHORTS
            print 'setDNA: which = %d, topStyle = %d, topColor = %d, botStyle = %d, botColor = %d' % (which, self.clothesGUI.topStyleChoice, self.clothesGUI.topColorChoice, self.clothesGUI.bottomStyleChoice, self.clothesGUI.bottomColorChoice)
            if self.roomAvailable == 0:
                if self.isLocalToon:
