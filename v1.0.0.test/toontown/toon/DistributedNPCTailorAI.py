        ...
        if avId in self.air.doId2do:
            ...
            if finished == 2 and which > 0:
                ...
                if canBuy:
                    ...
                    self.air.writeServerEvent('suspicious', avId, 'DistributedNPCTailorAI.setDNA bogus clothing ticket or jellybeans')
                    self.notify.warning('NPCTailor: setDNA() - client tried to purchase with bogus clothing ticket or jellybeans!')
                    if self.customerDNA:
                        av.b_setDNAString(self.customerDNA.makeNetString())
            elif finished == 1:
                if self.customerDNA:
