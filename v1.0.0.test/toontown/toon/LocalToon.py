import random, math, sys, time, re, zlib
from libotp import *
...
from toontown.shtiker import ServerPage
...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        try:
            ...
            if base.classicVisuals == 0:
                self.soundRun = base.loader.loadSfx('phase_3.5/audio/sfx/AV_footstep_runloop_new.ogg')
            ...
            self.taskDebugLabel = None
            self.pythonVersion = ('.').join([ str(id) for id in sys.version_info[:3] ])
            self.teleporting = False
            self.waitLabel = None
            self.waitSequence = None
            self.acceptingCars = False

        return

    def cleanupWaitLabel(self):
        if self.waitLabel:
            self.waitLabel.destroy()
        if self.waitSequence:
            self.waitSequence.pause()
        self.waitLabel = None
        self.waitSequence = None
        taskMgr.remove('waitLabel-task')
        return
    ...
    def acceptKartHooks(self):
        self.accept('insert', self.__startCarWait)
        self.accept('insert-up', self.__stopCarWait)
        self.accept('startMoving', self.__stopCarWait)
        self.acceptingCars = True

    def acceptOnscreenHooks(self):
        self.acceptKartHooks()

    def ignoreOnscreenHooks(self):
        self.cleanupWaitLabel()
        self.ignore('insert')
        self.ignore('insert-up')
        self.ignore('startMoving')
        self.acceptingCars = False

    def __startCarWait(self):
        self.cleanupWaitLabel()
        self.waitLabel = DirectFrame(parent=base.a2dTopCenter, relief=None, sortOrder=DGG.BACKGROUND_SORT_INDEX + 50, text='', text_scale=0.08, text_fg=(1,
                                                                                                                                                         1,
                                                                                                                                                         1,
                                                                                                                                                         1), text_shadow=(0,
                                                                                                                                                                          0,
                                                                                                                                                                          0,
                                                                                                                                                                          1), text_font=ToontownGlobals.getToonFont(), pos=(-0.05,
                                                                                                                                                                                                                            0,
                                                                                                                                                                                                                            -0.55), text_wordwrap=15)
        if self.carActive:
            self.waitLabel['text'] = TTLocalizer.HoldKartExitMessage
        else:
            if not self.hasKart():
                self.waitLabel['text'] = TTLocalizer.HoldKartNoKartMessage
                self.waitLabel['text_fg'] = (1, 0, 0, 1)
            else:
                if self.getHp() <= 10:
                    self.waitLabel['text'] = TTLocalizer.HoldKartLowLaffMessage
                    self.waitLabel['text_fg'] = (1, 0, 0, 1)
                else:
                    if not self.canRaceHere():
                        self.waitLabel['text'] = TTLocalizer.HoldKartCantRaceHereMessage
                        self.waitLabel['text_fg'] = (1, 0, 0, 1)
                    else:
                        self.waitLabel['text'] = TTLocalizer.HoldKartMessage
        if not self.hasKart() or not self.canRaceHere():
            self.waitSequence = Sequence(self.waitLabel.colorScaleInterval(1.25, 1, 0), Wait(1))
        else:
            self.waitSequence = Sequence(self.waitLabel.colorScaleInterval(1.25, 1, 0), Wait(1), Func(self.__carWaitOver))
        self.waitSequence.start()
        return

    def __stopCarWait(self):
        self.cleanupWaitLabel()

    def __carWaitOver(self):
        self.cleanupWaitLabel()
        self.carActive = not self.carActive
        self.d_requestCar(self.carActive)

    def d_requestCar(self, active):
        self.sendUpdate('requestCar', [active])
    ...
    def displayTalkWhisper(self, fromId, avatarName, rawString, mods, raw):
        sender = base.cr.identifyAvatar(fromId)
        if sender:
            chatString, scrubbed = sender.scrubTalk(rawString, mods, raw)
        else:
            chatString, scrubbed = self.scrubTalk(rawString, mods, raw)
        sender = self
    ...
    def loadFurnitureGui(self):
        ...
        self.__furnitureGui = DirectFrame(relief=None, parent=base.a2dTopLeft, pos=(0.143333,
                                                                                    0,
                                                                                    -0.67), scale=0.04, image=guiModels.find('**/attic'))
    ...
    def __updateTaskDebugLabel(self, task):
        debug = str(taskMgr)
        if self.taskDebugLabel['text'] != debug:
            self.taskDebugLabel['text'] = debug
        return task.again

    def __updateIvalDebugLabel(self, task):
        debug = str(ivalMgr)
        if self.taskDebugLabel['text'] != debug:
            self.taskDebugLabel['text'] = debug
        return task.again
    ...
    def stopTaskDebug(self):
        if self.taskDebugLabel:
            self.taskDebugLabel.destroy()
        self.taskDebugLabel = None
        taskMgr.remove(self.uniqueName('updateTaskDebugLabel'))
        return

    def createTaskLabel(self, text):
        if self.taskDebugLabel:
            self.stopTaskDebug()
            return False
        self.taskDebugLabel = DirectLabel(base.a2dTopLeft, relief=None, text=text, text_font=ToontownGlobals.getToonFont(), text_scale=0.036, text_align=TextNode.ALeft, text_fg=(1,
                                                                                                                                                                                  1,
                                                                                                                                                                                  1,
                                                                                                                                                                                  1), text_shadow=(0,
                                                                                                                                                                                                   0,
                                                                                                                                                                                                   ...
                                                                                                                                                                                                   1), pos=(0.03,
                                                                                                                                                                                                            0,
                                                                                                                                                                                                            -0.05))
        self.taskDebugLabel.setBin('gui-popup', 0)
        self.taskDebugLabel.show()
        return True

    def thinkTask(self):
        if self.createTaskLabel(str(taskMgr)):
            taskMgr.doMethodLater(1.0, self.__updateTaskDebugLabel, self.uniqueName('updateTaskDebugLabel'))

    def thinkIval(self):
        if self.createTaskLabel(str(ivalMgr)):
            taskMgr.doMethodLater(1.0, self.__updateIvalDebugLabel, self.uniqueName('updateTaskDebugLabel'))

    def thinkPos(self):
        if self.debugLabel:
            self.stopDebug()
            return
        self.debugLabel = DirectLabel(base.a2dBottomLeft, relief=None, text=self.getDebugInfo(), text_font=ToontownGlobals.getToonFont(), text_scale=0.05, text_align=TextNode.ALeft, text_fg=(1,
                                                                                                                                                                                               1,
                                                                                                                                                                                               1,
                                                                                                                                                                                               1), text_shadow=(0,
                                                                                                                                                                                                                0,
    ...
    def refreshOnscreenButtons(self):
        ...
        if self.moveFurnitureButtonObscured <= 0:
            if self.furnitureManager != None and self.furnitureDirector == self.doId:
                ...
                taskMgr.remove('lerpFurnitureButton')
                self.__furnitureGui.posHprScaleInterval(pos=Point3(0.143333, 0, -0.67), hpr=Vec3(0.0, 0.0, 0.0), scale=Vec3(0.04, 0.04, 0.04), duration=1.0, blendType='easeInOut', name='lerpFurnitureButton').start()
    ...
    def setSpeed(self, forwardSpeed, rotateSpeed):
        ...
        if self.standWalkRunReverse != None and base.classicVisuals == 0:
            replay = 0
            showWake, wakeWaterHeight = ZoneUtil.getWakeInfo()
            if showWake and self.getZ(render) < wakeWaterHeight and not self.useWaterSound:
                ...
                self.soundRun = base.loader.loadSfx('phase_4/audio/sfx/AV_footstep_runloop_new_water.ogg')
                ...
                if not (showWake and self.getZ(render) < wakeWaterHeight) and self.useWaterSound:
                    if self.soundRun.status() == self.soundRun.PLAYING:
                        replay = 1
                        self.soundRun.stop()
                    base.loader.unloadSfx(self.soundRun)
