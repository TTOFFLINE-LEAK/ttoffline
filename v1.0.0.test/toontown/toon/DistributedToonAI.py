...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def announceZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.air.welcomeValleyManager:
    ...
    def setNPCFriendsDict(self, NPCFriendsList):
        ...
        for friendPair in NPCFriendsList:
            npcFriends = NPCToons.loadCards(returnDict=True, cardInfo=json.loads(simbase.air.moddingManager.getCards()))
    ...
    def takeDamage(self, hpLost, quietly=0, sendTotal=1):
        ...
        if not self.hpOwnedByBattle:
            ...
            if self.hp <= 10 and self.getCarActive():
                if self.kart:
                    self.kart.requestDelete()
                    self.kart = None
                self.b_setCarActive(False)
            if sendTotal:
    ...
    def b_setCogIndex(self, index, cogType=0):
        self.setCogIndex(index)
        self.d_setCogIndex(index, cogType)
    ...
    def d_setCogIndex(self, index, cogType=0):
        self.sendUpdate('setCogIndex', [index, cogType])

    def getCogIndex(self):
        return self.cogIndex

    def d_setCharIndex(self, index):
        if index == -1:
            self.isClassicChar = 0
        else:
            ...
            self.isGoon = 0
            self.isBossCog = 0
        self.sendUpdate('setCharIndex', [index])
    ...
    def setTotalMoney(self, cost):
        if self.getTotalMoney() >= cost:
            bankPrice = min(self.getBankMoney(), cost)
            walletPrice = cost - bankPrice
            self.b_setBankMoney(self.getBankMoney() - bankPrice)
            self.b_setMoney(self.getMoney() - walletPrice)
            return True
        return False
    ...
    def d_setSCResistance(self, msgIndex, nearbyPlayers):
        ...
        for toonId in nearbyPlayers:
            ...
            elif msgType == ResistanceChat.RESISTANCE_GREEN and self.getAccessLevel() < 200:
                self.notify.warning('%s said toon-down resistance but is not a moderator' % self.doId)
            else:
    ...
    def doResistanceEffect(self, msgIndex):
        ...
        elif msgType == ResistanceChat.RESISTANCE_GREEN:
            if msgValue == -1:
    ...
    if simbase.wantPets:
        ...
        def setSC(self, msgId):
            ...
            if msgId in (21006, ):
                self.setHatePets(1)
            elif msgId in (21000, 21001, 21003, 21004, 21200, 21201, 21202, 21203,
                           21204, 21205, 21206):
                self.setHatePets(0)
    ...
    def tryToUseSpecial(self, special):
        ...
        if estateOwnerDoId:
            ...
            if estate and hasattr(estate, 'idList'):
                ownerIndex = estate.idList.index(estateOwnerDoId)
                if ownerIndex >= 0:
    ...
    def b_setGM(self, type, updateName=False):
        self.sendUpdate('setGM', [type])
        self.setGM(type, updateName)

    def setGM(self, type, updateName=False):
        wasGM = self._isGM
    ...
    def setName(self, name):
        ...
        if config.GetBool('want-kick-inappropriate-name', True):
            nameList = name.lower().split(' ')
            for blockedName in self.blockNames:
                if blockedName in nameList:
                    message = '%s found in toon name: %s' % (blockedName, name)
                    self.notify.warning(message + ' | %s' % self.doId)
                    self.air.writeServerEvent('suspicious', self.doId, message)
                    self.sendSetKick('Inappropriate name detected, Your name has been revoked.', self)
                    self.b_setName('Toon')
                    return
    ...
    def checkTimeout(self, timeoutName, timeout):
        if timeoutName in self.timeouts and self.timeouts[timeoutName] > time.time():
            return False
        else:
            self.timeouts[timeoutName] = time.time() + timeout
            return True

    def b_setCarActive(self, carActive):
        self.setCarActive(carActive)
        self.d_setCarActive(carActive)

    def d_setCarActive(self, carActive):
        self.sendUpdate('setCarActive', [carActive])
    ...
    def requestCar(self, active):
        if self.carActive == active or not self.hasKart():
            return
        if not self.checkTimeout('requestCar', 2.5):
            return
        else:
            self.b_setCarActive(active)
            if self.carActive:
                if not self.kart:
                    from toontown.racing import DistributedVehicleAI
                    self.kart = DistributedVehicleAI.DistributedVehicleAI(self.air, self.doId)
                    self.kart.setControlled(True)
                    self.kart.generateWithRequiredAndId(self.air.allocateChannel(), self.doId, 100)
                    self.kart.d_setNonRace(True)
                    self.kart.start()
            elif self.kart:
                self.kart.requestDelete()
                self.kart = None
    ...
    def sendSetBan(self, reason='', target=None, invoker=None, duration=0):
        self.air.banManager.sendBan(sender=target.DISLid, avId=target.getDoId(), reason=reason, duration=duration)

    def sendSetKick(self, reason='', target=None, invoker=None, silent=0):
        self.air.banManager.sendKick(avId=target.getDoId(), reason=reason, silent=silent)

    def d_generateRainbow(self):
        self.sendUpdateToAvatarId(self.doId, 'generateRainbow', [])

    def d_generateFanfare(self):
        self.sendUpdate('generateFanfare', [])

    def d_generateTrolley(self):
        self.sendUpdate('generateTrolley', [globalClockDelta.getRealNetworkTime()])

    def d_generateBrowserEasterEgg(self, index):
        self.sendUpdateToAvatarId(self.doId, 'generateBrowserEasterEgg', [index])

    def d_generateGreenEffect(self, character, toonId):
        self.sendUpdate('generateGreenEffect', [character, toonId])

    def d_generateSnapEffect(self):
        self.sendUpdate('generateSnapEffect', [])

    def d_generateOboeEffect(self):
        self.sendUpdate('generateOboeEffect', [])

    def d_generateCage(self):
        self.sendUpdate('generateCage', [])
    ...
    def b_setMuted(self, muted, timed):
        self.setMuted(muted)
        self.d_setMuted(muted, timed)
        if muted and timed:
            taskMgr.doMethodLater(timed * 60, self.b_setMuted, self.uniqueName('unmute'), extraArgs=[False, 0])

    def d_setMuted(self, muted, timed):
        self.sendUpdate('setMuted', [muted, timed])
    ...
    def d_setLoop(self, anim, start, end, part):
        self.sendUpdate('setLoop', [anim, start, end, part])

    def d_setPingPong(self, anim, start, end, part):
        self.sendUpdate('setPingPong', [anim, start, end, part])

    def d_setPose(self, anim, frame, part):
        self.sendUpdate('setPose', [anim, frame, part])

    def b_setAccessLevel(self, access):
        self.air.dbInterface.updateObject(self.air.dbId, self.DISLid, self.air.dclassesByName['AccountAI'], {'ACCESS_LEVEL': access})
