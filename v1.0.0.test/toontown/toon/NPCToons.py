...
def createNPC(air, npcId, desc, zoneId, posIndex=0, questCallback=None):
    import DistributedNPCToonAI, DistributedNPCClerkAI, DistributedNPCTailorAI, DistributedNPCBlockerAI, DistributedNPCFishermanAI, DistributedNPCPetclerkAI, DistributedNPCKartClerkAI, DistributedNPCPartyPersonAI, DistributedNPCSpecialQuestGiverAI, DistributedNPCFlippyInToonHallAI, DistributedNPCScientistAI, DistributedNPCKongAI, DistributedNPCMagicCatAI
    if len(desc) == 6:
...
HQnpcFriends = {2001: (ToontownBattleGlobals.HEAL_TRACK,
   ...
   4140: (
        ...
        3), 
   3137: (
        ToontownBattleGlobals.NPC_COGS_MISS,
        0,
        0,
        4), 
   4327: (
   ...
   4230: (
        ...
        4), 
   3135: (
        ToontownBattleGlobals.NPC_TOONS_HIT,
        0,
        0,
        4), 
   2208: (
...
FOnpcFriends = {9310: (ToontownBattleGlobals.LURE_TRACK,
   ...
   9303: (
        ...
        30,
        2)}
exclusiveNpcFriends = {4141: (ToontownBattleGlobals.HEAL_TRACK,
        1,
        ToontownGlobals.MaxHpLimit,
        5)}
npcFriends = dict(HQnpcFriends)
npcFriends.update(FOnpcFriends)
npcFriends.update(exclusiveNpcFriends)
...
def getNPCTrack(npcId):
    cardInfo = None
    if not PythonUtil.isClient():
        cardInfo = json.loads(simbase.air.moddingManager.getCards())
...
def getNPCToonsNames():
    names = loadCards(returnNames=True)
    return names


def loadCards(returnDict=False, returnNames=False, cardInfo=None, name=None):
    if not cardInfo:
        cards = base.cr.moddingManager.getCards()
    else:
        cards = cardInfo
    for i in xrange(len(cards['name'])):
        TTLocalizer.NPCToonNames[cards['id'][i]] = str(cards['name'][i])
        if len(cards) < 27:
            NPCToonDict[cards['id'][i]] = (cards['zone'][i],
             cards['name'][i],
             (
              cards['head'][i],
              cards['torso'][i],
              ...
              cards['shirt-color'][i],
              cards['sleeve-id'][i],
              cards['sleeve-color'][i],
              cards['bottoms-id'][i],
              cards['bottoms-color'][i]),
             cards['gender'][i],
             cards['has-building'][i],
             cards['type'][i])
        else:
            NPCToonDict[cards['id'][i]] = (cards['zone'][i],
             cards['name'][i],
    ...
    if returnDict:
        return npcFriends
    if returnNames:
        return TTLocalizer.NPCToonNames
    if cardInfo:
