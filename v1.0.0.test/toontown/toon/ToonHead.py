...
class ToonHead(Actor.Actor):
    ...
    def generateToonColor(self, style):
        ...
        if not self.fullToon and style.headColor == 27:
            ...
            if lp.isEmpty() or rp.isEmpty():
                ...
                if base.classicVisuals in (0, 2):
                    if style.head[0] + style.head[1] == 'ds':
                        ...
                        rpt.setScale(0.9)
                    if base.classicVisuals == 0 and style.getAnimal() == 'dog':
                        lpt.setTexScale(TextureStage.getDefault(), -1, 1, 1)
                        lpt.setTexOffset(TextureStage.getDefault(), 0.09375, 0)
                lp.reparentTo(lpt)
                rp.reparentTo(rpt)
    ...
    def __fixHeadLongShort(self, style, lodName=None, copy=1):
        ...
        if lodName == None:
            ...
            searchRoot = self.find('**/' + str(lodName))
        if animalType not in ('duck', 'horse', 'gyro', 'scrooge'):
    ...
    def surprisedEyes(self):
        self.__eyesOpen = ToonHead.EyesSurprised
        self.__eyesClosed = ToonHead.EyesSurprised
