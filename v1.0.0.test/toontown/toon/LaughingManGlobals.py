...
def makeCard(book=False):
    ...
    lmBase.setTexture(loader.loadTexture('phase_14/maps/lm_base.png'))
    lmBase.setY(-0.3)
    lmBase.setTransparency(True)
    lmText = nodePath.attachNewNode(cardMaker.generate())
    lmText.setTexture(loader.loadTexture('phase_14/maps/lm_text.png'))
    lmText.setY(-0.301)
    lmText.setTransparency(True)
    lmText.hprInterval(10, (0, 0, -360)).loop()
    lmFace = nodePath.attachNewNode(cardMaker.generate())
    lmFace.setTexture(loader.loadTexture('phase_14/maps/lm_face.png'))
    lmFace.setY(-0.302)
