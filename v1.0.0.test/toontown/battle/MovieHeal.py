...
def __healJoke(heal, hasInteractivePropHealBonus):
    ...
    level = heal['level']
    if npcId == 4141:
        jokes = HealJokes.kongHealJokes
        jokeIndex = random.choice(xrange(len(HealJokes.kongHealJokes)))
    else:
        jokes = HealJokes.toonHealJokes
    ...
    tracks.append(Sequence(Wait(tSpeakPunchline), Func(toon.clearChat), Func(toon.setChatAbsolute, joke[1], CFSpeech | CFTimeout)))
    ...
    if npcId != 0:
        tracks.append(Sequence(Wait(tRunBack), Func(toon.clearChat), *MovieNPCSOS.teleportOut(heal, toon)))
    else:
