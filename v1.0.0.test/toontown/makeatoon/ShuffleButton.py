...
class ShuffleButton:
    ...
    def load(self):
        ...
        self.shuffleBtn = DirectButton(parent=self.parentFrame, relief=None, image=(
         shuffleUp, shuffleDown, shuffleUp, shuffleDown), image_scale=halfButtonInvertScale, image1_scale=(-0.63,
                                                                                                           0.6,
                                                                                                           0.6), image2_scale=(-0.63,
                                                                                                                               0.6,
                                                                                                                               0.6), text=TTLocalizer.ShuffleButton, text_font=ToontownGlobals.getInterfaceFont(), text_scale=TTLocalizer.SBshuffleBtn, text_pos=(0,
                                                                                                                                                                                                                                                                  -0.02), text_fg=(1,
                                                                                                                                                                                                                                                                                   1,
                                                                                                                                                                                                                                                                                   1,
                                                                                                                                                                                                                                                                                   1), text_shadow=(0,
        ...
        self.frameHideLerp = LerpColorInterval(self.shuffleFrame, self.lerpDuration, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1))
        self.incBtnHideLerp = LerpColorInterval(self.incBtn, self.lerpDuration, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1))
        self.decBtnHideLerp = LerpColorInterval(self.decBtn, self.lerpDuration, Vec4(1, 1, 1, 0), Vec4(1, 1, 1, 1))
        self.__updateArrows()
        return
    ...
    def startShowLerp(self):
        self.shuffleBtn['state'] = DGG.NORMAL
        if len(self.history) >= 2:
            timestamp = 0
            if self.hideLerp.isPlaying():
                timestamp = self.hideLerp.getT()
                self.hideLerp.finish()
            self.shuffleFrame.show()
            self.incBtn.show()
            self.decBtn.show()
            self.showLerp = Sequence(Parallel(self.frameShowLerp, self.incBtnShowLerp, self.decBtnShowLerp))
            self.showLerp.start()
            if timestamp:
                self.showLerp.setT(self.showLerp.getDuration() - timestamp)

    def startHideLerp(self):
        self.shuffleBtn['state'] = DGG.DISABLED
        if len(self.history) >= 2:
            timestamp = 0
            if self.showLerp.isPlaying():
                timestamp = self.showLerp.getT()
                self.showLerp.finish()
            self.incBtn.hide()
            self.decBtn.hide()
            self.hideLerp = Sequence(Parallel(self.frameHideLerp, self.incBtnHideLerp, self.decBtnHideLerp), Func(self.shuffleFrame.show))
            self.hideLerp.start()
            if timestamp:
                self.hideLerp.setT(self.showLerp.getDuration() - timestamp)
