...
class DistributedCannonGame(DistributedMinigame):
    ...
    def __calcFlightResults(self, avId, launchTime):
        ...
        return {'startPos': startPos, 'startHpr': startHpr, 
           'startVel': startVel, 
    ...
    def __flyToBackOfCannon(self, task):
        ...
        if self.__cameraLookAtInterval:
            self.__cameraLookAtInterval.finish()
        self.__cameraLookAtInterval = Parallel(lerpNode.hprInterval(self.T_TOONHEAD2CANNONBACK, endRotation, blendType='easeInOut'), camera.posInterval(self.T_TOONHEAD2CANNONBACK, endPos, blendType='easeInOut'), name=self.INTRO_TASK_NAME_CAMERA_LERP)
        self.__cameraLookAtInterval.start()
        return Task.done