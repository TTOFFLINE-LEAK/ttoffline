...
class DistributedFishingTargetAI(DistributedNodeAI):
    ...
    def getState(self):
        return [
         0, self.angle, self.radius, self.time, globalClockDelta.getRealNetworkTime()]

    def __updateState(self, task=None):
        self.b_setPosHpr(self.radius * math.cos(self.angle) + self.centerPoint[0], self.radius * math.sin(self.angle) + self.centerPoint[1], self.centerPoint[2], 0, 0, 0)
