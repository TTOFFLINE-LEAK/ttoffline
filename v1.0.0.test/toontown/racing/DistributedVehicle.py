...
class DistributedVehicle(DistributedSmoothNode.DistributedSmoothNode, Kart.Kart, FSM.FSM):
    ...
    turnRatio = 1.7 / 0.025 * math.sqrt(cheatFactor)
    accelerationMult = 35
    accelerationBase = 26
    if proRacer:
        accelerationMult = 35
    ...
    def announceGenerate(self):
        ...
        self.generateKart()
        self.toon = base.cr.doId2do.get(self.ownerId)
        self.toon.kart = self
        ...
        base.cr.karts[self.ownerId] = self
        if self.localVehicle:
            if self.controlled:
                if base.cr.playGame.place.fsm.getCurrentState().getName() != 'stopped':
                    base.cr.playGame.place.fsm.request('stopped')
                    base.localAvatar.acceptKartHooks()
                self.setPos(base.localAvatar.getPos(render))
                self.setHpr(base.localAvatar.getHpr(render))
                self.reparentTo(render)
                base.localAvatar.setPreventSleepWatch(True)
        if self.controlled:
            self.geom[0].setScale(0.85)
            self.accelerationBase = 10
        if self.localVehicle:
            self.setupPhysics()
            self.setupParticles()
    ...
    def disable(self):
        if self.controlled and self.localVehicle:
        ...
        self.stopSkid()
        self.stopSmooth()
        if self.ownerId in base.cr.karts:
            del base.cr.karts[self.ownerId]
        if self.localVehicle:
    ...
    def __wallCollisionStart(self, entry):
        if self.controlled:
            intoPath = entry.getIntoNodePath()
            name = intoPath.getName()
            if name.startswith('tunnel_trigger_'):
                zoneId = int(name.split('_')[(-1)])
                base.cr.playGame.place.tunnelOutSkip(base.localAvatar.zoneId, zoneId)
                return
            if 'bellicose-' in name:
                intoPath = intoPath.getParent()
                if not intoPath.isEmpty():
                    intoPath.hide()
                doId = int(name.split('-')[(-1)])
                if doId not in self.exploded:
                    self.d_requestExplosion(doId)
                    self.exploded.append(doId)
        self.entry = entry
    ...
    def d_requestExplosion(self, doId):
        self.sendUpdate('requestExplosion', [doId])

    def explosion(self):
        kapow = globalPropPool.getProp('kapow')
        kapow.setBillboardPointEye()
        kapow.reparentTo(render)
        kapow.setPos(self, 0, 0, 4)
        kapow.setScale(0.4)
        explodeSfx = loader.loadSfx('phase_3.5/audio/sfx/ENC_cogfall_apart.ogg')
        sequence = Sequence(Func(base.playSfx, explodeSfx), ActorInterval(kapow, 'kapow'), Func(kapow.delete))
        sequence.start()
    ...
    def enterControlled(self):
        ...
        if self.localVehicle:
            ...
            self.startSmooth()
            taskName = 'updateNonlocalVehicle-%s' % self.ownerId
    ...
    def exitControlled(self):
        ...
        if self.toon and not self.toon.isDisabled() and not self.toon.isEmpty():
            ...
            self.toon.setPos(self.geom[0].getPos(render))
            self.toon.setHpr(self.geom[0].getHpr(render))
    ...
    def __enableControlInterface(self):
        ...
        if not self.speedometer:
            ...
            self.speedometer = DirectLabel(relief=None, parent=base.a2dBottomRight, pos=(-0.0933334,
                                                                                         0,
                                                                                         0.02), text=str(0), text_scale=0.18, text_fg=bodyColor, text_pos=(-0.04,
                                                                                                                                                           0.02,
    ...
    def startTurbo(self):
        ...
        cameraZoomIn = Parallel(LerpPosInterval(camera, 2, newCameraPos), LerpFunc(base.camLens.setMinFov, fromData=startFov, toData=newCameraFov / (4.0 / 3.0), duration=2))
        cameraToNormal = Parallel(LerpPosInterval(camera, 1, Point3(0, -33, 16), newCameraPos), LerpFunc(base.camLens.setMinFov, fromData=newCameraFov / (4.0 / 3.0), toData=ToontownGlobals.DefaultCameraFov / (4.0 / 3.0), duration=1))
    ...
    def __animate(self):
        ...
        if self.localVehicle and not self.controlled:
    ...
    def stopCarImmediately(self):
        self.actorNode.getPhysicsObject().setVelocity(Vec3(0, 0, 0))

    def disableForSeconds(self, seconds):
        self.stopped = True
        taskMgr.doMethodLater(seconds, self.startCar, 'startCar-task', extraArgs=[])
    ...
    def setOwner(self, avId):
        self.ownerId = avId
        base.cr.karts[avId] = self

    def stopCar(self, level):
        self.imHitMult = level
        if hasattr(self, 'cameraTrack') and self.cameraTrack:
            self.cameraTrack.pause()
            cameraToNormal = Parallel(LerpPosInterval(camera, 0.05, Point3(0, -33, 16), startPos=camera.getPos()), LerpFunc(base.camLens.setMinFov, fromData=base.camLens.getMinFov(), toData=ToontownGlobals.DefaultCameraFov / (4.0 / 3.0), duration=0.05))
