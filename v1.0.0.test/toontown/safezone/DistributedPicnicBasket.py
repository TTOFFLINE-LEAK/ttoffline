...
class DistributedPicnicBasket(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM('DistributedPicnicBasket', [State.State('off', self.enterOff, self.exitOff, ['waitEmpty', 'waitCountdown']), State.State('waitEmpty', self.enterWaitEmpty, self.exitWaitEmpty, ['waitCountdown']), State.State('waitCountdown', self.enterWaitCountdown, self.exitWaitCountdown, ['waitEmpty'])], 'off', 'off')
    ...
    def fillSlot(self, index, avId):
        ...
        if avId == base.localAvatar.getDoId():
            ...
            if hasattr(self.loader.place, 'picnic'):
                self.loader.place.picnic.fsm.request('boarding', [self.tablecloth, side])
            else:
        ...
        if avId in self.cr.doId2do:
            ...
            track.append(Sequence(Func(self.clearToonTrack, avId), name=toon.uniqueName('fillPicnic'), autoPause=1))
            if avId == base.localAvatar.getDoId():
                if hasattr(self.loader.place, 'picnic'):
                    track.append(Func(self.loader.place.picnic.exitButton.show))
