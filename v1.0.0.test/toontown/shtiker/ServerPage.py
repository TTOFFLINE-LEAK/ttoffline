...
from toontown.server import MiniserverIcon
POP_COLORS_NTT = (
...
class ServerPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.currentTab = DirectButton(parent=self, relief=None, text=TTLocalizer.ServerTabTitle, text_scale=TTLocalizer.OPoptionsTab, text_align=TextNode.ALeft, text_pos=(0.01,
                                                                                                                                                                            0.0,
...
class ServerTabPage(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, _parent=None):
        ...
        self.unText = DirectLabel(parent=self, relief=None, text='Username', text_scale=0.05, text_align=TextNode.ACenter, pos=(-0.45,
                                                                                                                                0,
                                                                                                                                -0.275))
        self.unBox = DirectFrame(parent=self, relief=None, image=cdrGui.find('**/tt_t_gui_sbk_cdrCodeBox2'), pos=(-0.45,
                                                                                                                  0,
                                                                                                                  -0.375), scale=0.6)
        ...
        self.unInput.enterText(base.cr.playToken)
        self.ipText = DirectLabel(parent=self, relief=None, text='IP Address', text_scale=0.05, text_align=TextNode.ACenter, pos=(-0.45,
                                                                                                                                  0,
                                                                                                                                  -0.5))
        self.ipBox = DirectFrame(parent=self, relief=None, image=cdrGui.find('**/tt_t_gui_sbk_cdrCodeBox2'), pos=(-0.45,
                                                                                                                  0,
                                                                                                                  -0.6), scale=0.6)
    ...
    def enter(self):
        self.updateMiniserverList()
        taskMgr.doMethodLater(15, self.updateMiniserverList, 'updateList', extraArgs=[])
        self.show()
    ...
    def getServersFromAPI(self, id=''):
        ...
        request = urllib2.Request('http://localhost/api/miniservers/%s' % id, headers=headers)
    ...
    def createMiniserverButton(self, servers, currentWordIndex, condition=False, updating=False):
        ...
        miniServerButton = DirectButton(parent=self, relief=None, text=servers[currentWordIndex]['name'], text_align=TextNode.ALeft, text_scale=0.05, text1_bg=self.textDownColor, text2_bg=self.textRolloverColor, text3_fg=self.textDisabledColor, textMayChange=0, command=self.showServerInfo, extraArgs=[
         currentWordIndex])
        nameLabel = DirectLabel(parent=hidden, relief=None, text=servers[currentWordIndex]['name'], text_align=TextNode.ACenter, text_scale=0.06, text_wordwrap=12, pos=(0.45,
                                                                                                                                                                         0,
                                                                                                                                                                         0.425))
        ...
        connectButton = DirectButton(parent=hidden, image=(
         gui.find('**/QuitBtn_UP'), gui.find('**/QuitBtn_DN'),
         gui.find('**/QuitBtn_RLVR'), gui.find('**/QuitBtn_RLVR')), image_scale=1.15, relief=None, text='Play', text_scale=TTLocalizer.OPCodesSubmitTextScale, text_pos=(0,
                                                                                                                                                                         -0.02), scale=1, pos=(0.45,
                                                                                                                                                                                               0,
                                                                                                                                                                                               -0.6), command=self.connectDialog, extraArgs=[
         self.ipInput.get(), servers[currentWordIndex]['name']])
        ...
        if servers[currentWordIndex] not in self.old.values():
            self.old[miniServerButton] = servers[currentWordIndex]
        if updating:
    ...
    def generateServerIcon(self, servers, currentWordIndex):
        icon = None
        serverName = servers[currentWordIndex]['name']
        serverIconPath = 'http://localhost/images/server_icons/' + servers[currentWordIndex]['image_src']
        serverIcon = MiniserverIcon.MiniserverIcon(serverName, serverIconPath)
        if serverIcon:
            iconTexturePath = serverIcon.IconsBaseDir + '/' + serverIcon.iconFileName
            loaderTexturePath = Filename(str(iconTexturePath))
            if loaderTexturePath and Filename(loaderTexturePath).exists():
                icon = OnscreenImage(parent=hidden, image=iconTexturePath, pos=(0.45,
                                                                                0,
                                                                                0.15), scale=(0.4,
                                                                                              0.4,
                                                                                              0.23275))
        ...
        if icon:
            server = self.indexToButton[currentWordIndex]
            self.miniservers[server].append(icon)
        return
            ...
            self.scrollList = DirectScrolledList(parent=self, forceHeight=0.07, pos=(-0.5,
             ...
             gui.find('**/FndsLst_ScrollUp')), incButton_relief=None, incButton_scale=(1.3,
                                                                                       ...
                                                                                       -1.3), incButton_pos=(0.08,
                                                                                                             0,
                                                                                                             -0.2), incButton_image3_color=Vec4(1, 1, 1, 0.2), decButton_image=(
             gui.find('**/FndsLst_ScrollUp'),
             gui.find('**/FndsLst_ScrollDN'),
             gui.find('**/FndsLst_ScrollUp_Rllvr'),
    ...
    def showServerInfo(self, serverNum):
        ...
        for server in self.miniservers.keys():
            ...
            if self.miniservers[server][3]['state'] == DGG.DISABLED:
                ...
                self.ipInput.focusOutCommandFunc()
                ...
                self.ipInput.focusInCommandFunc()
        ...
        self.ipInput.enterText(str(self.old[button]['ip']))
    ...
    def openMoreInformation(self, value):
        ...
        del self.askDialog
        if value == DGG.DIALOG_OK:
            webbrowser.open('http://localhost/servers/' + str(self.serverId))

    def updateMiniserverList(self):
...
class CurrentTabPage(DirectFrame):
    ...
    def load(self):
        self.totalPopulationText = DirectLabel(parent=self, relief=None, text=TTLocalizer.ServerPagePopulation.format(base.cr.discordManager.getDistrictAvatarCount(), base.cr.discordManager.getDistrictLimit()), text_scale=0.08, text_wordwrap=24, textMayChange=1, text_align=TextNode.ACenter, pos=(0,
                                                                                                                                                                                                                                                                                                         0,
                                                                                                                                                                                                                                                                                                         0.4))
        ...
        self.titleText = DirectLabel(parent=self, relief=None, text=TTLocalizer.ServerPageName % base.cr.serverName, text_scale=0.08, text_wordwrap=24, textMayChange=1, text_align=TextNode.ACenter, pos=(0,
                                                                                                                                                                                                           ...
                                                                                                                                                                                                           0.5))
        self.titleText.show()
        self.descText = DirectLabel(parent=self, relief=None, text=TTLocalizer.ServerPageDesc % base.cr.serverDescription, text_scale=0.07, text_wordwrap=24, textMayChange=1, text_align=TextNode.ACenter, pos=(0,
                                                                                                                                                                                                                 0,
                                                                                                                                                                                                                 0.3))
        self.descText.show()
        ...
        self.unBox = DirectFrame(parent=self, relief=None, image=cdrGui.find('**/tt_t_gui_sbk_cdrCodeBox2'), pos=(unX, 0, -0.425), scale=0.8)
        self.unInput = DirectEntry(parent=self.unBox, relief=DGG.GROOVE, scale=0.08, pos=(-0.33,
                                                                                          0,
                                                                                          -0.006), borderWidth=(0.05,
                                                                                                                0.05), frameColor=(
         (1, 1, 1, 1), (1, 1, 1, 1), (0.5, 0.5, 0.5, 0.5)), state=DGG.NORMAL, text_align=TextNode.ALeft, text_scale=TTLocalizer.OPCodesInputTextScale, width=10.5, numLines=1, focus=1, backgroundFocus=0, cursorKeys=1, text_fg=(0,
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  1), suppressMouse=1, autoCapitalize=0)
        if config.GetBool('mini-server', False):
            self.ipText = DirectLabel(parent=self, relief=None, text='IP Address', text_scale=0.07, text_align=TextNode.ACenter, pos=(0.433,
                                                                                                                                      0,
                                                                                                                                      -0.3))
            self.ipBox = DirectFrame(parent=self, relief=None, image=cdrGuiO.find('**/tt_t_gui_sbk_cdrCodeBox2'), pos=(0.433,
                                                                                                                       0,
                                                                                                                       -0.425), scale=0.8)
            self.ipInput = DirectEntry(parent=self.ipBox, relief=DGG.GROOVE, scale=0.08, pos=(-0.33,
                                                                                              0,
    ...
    def updatePopulation(self):
        ...
        if totalPop == 0:
            totalPop = 16
        self.totalPopulationText['text'] = TTLocalizer.ServerPagePopulation.format(totalPop, base.cr.discordManager.getDistrictLimit())
        if self.button:
            self.button.destroy()
