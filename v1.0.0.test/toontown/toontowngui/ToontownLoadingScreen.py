...
class ToontownLoadingScreen:
    ...
    def __init__(self):
        ...
        if base.classicVisuals == 2:
            self.banner = DirectLabel(parent=self.gui, relief=None, sortOrder=0, image=loader.loadTexture('phase_3.5/maps/trolley.jpg', 'phase_3.5/maps/trolley_a.rgb'), scale=0.625)
            self.banner.setTransparency(TransparencyAttrib.MAlpha)
            self.logo = DirectLabel(parent=self.gui, relief=None, sortOrder=1, image=self.gui.find('**/logo'))
    ...
    def begin(self, range, label, gui, tipCategory):
        self.gui.find('**/bg').setScale(base.getAspectRatio(), 1, 1)
