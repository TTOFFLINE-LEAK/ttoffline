...
SUITE_DICT = {'s': 'tt_m_ara_crg_penthouse', 'l': 'tt_m_ara_crg_penthouseLawbot', 'm': 'tt_m_ara_crg_penthouse', 
...
class DistributedCogdoInterior(DistributedObject.DistributedObject):
    ...
    def announceGenerate(self):
        ...
        base.cr.discordManager.setInfo(base.cr.discordManager.getState(), TTLocalizer.DiscordFieldOffice, ToontownGlobals.Zone2String.get(ZoneUtil.getHoodId(self.zoneId)), None, None, None, self.zoneId)
        return
    ...
    def __outroPenthouseChatDone(self, elapsed=None):
        ...
        track = Parallel(Sequence(ActorInterval(self.shopOwnerNpc, 'wave'), Func(self.shopOwnerNpc.loop, 'neutral')), Sequence(Wait(2.0), Func(self.exitCogdoBuilding), Func(base.camLens.setMinFov, ToontownGlobals.DefaultCameraFov / (4.0 / 3.0))))
        track.start()
