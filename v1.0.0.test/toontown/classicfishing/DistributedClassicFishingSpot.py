...
class DistributedClassicFishingSpot(DistributedObject.DistributedObject):
    ...
    def setMovie(self, mode, code, item, speed):
        ...
        if mode == FishingCodes.EnterMovie:
            ...
            if mode == FishingCodes.ExitMovie:
                if self.localToonFishing:
                    self.__hideGui()
                self.av.stopLookAround()
                ...
                if mode == FishingCodes.CastMovie:
                    self.av.stopLookAround()
                    self.av.startLookAround()
                    self.__placeAvatar()
                    self.__getBobSpot()
                    ...
                    if mode == FishingCodes.NibbleMovie:
                        self.__placeAvatar()
                        ...
                        if mode == FishingCodes.BeginReelMovie:
                            self.av.stopLookAround()
                            self.__placeAvatar()
                            self.__hideBob()
                            ...
                            if mode == FishingCodes.ContinueReelMovie:
                                self.av.stopLookAround()
                                self.__showLineReelTaught()
                                ...
                                if mode == FishingCodes.PullInMovie:
                                    self.av.startLookAround()
                                    self.__placeAvatar()
                                    self.__hideBob()
