...
import AvatarDNA
from toontown.battle.BattleProps import globalPropPool, Variants
...
class DistributedAvatar(DistributedActor, Avatar):
    ...
    def setLoop(self, loop, start, end, partName):
        if partName == '':
            partName = None
        if start == -1:
            start = None
        ...
        self.loop(loop, partName=partName, fromFrame=start, toFrame=end)
        return

    def setPose(self, anim, frame, partName):
        if partName == '':
            partName = None
        self.pose(anim, frame, partName)
        return

    def setPingPong(self, anim, start, end, partName):
        if partName == '':
            partName = None
        if start == -1:
            start = None
        ...
        self.pingpong(anim, partName=partName, fromFrame=start, toFrame=end)
        return

    def setLeftHand(self, prop):
        leftHand = self.find('**/leftHand')
        if prop == '':
    ...
    def spawnProp(self, prop):
        allowedZones = []
        for baseZone in ToontownGlobals.HoodHierarchy:
            allowedZones.append(baseZone)
            streetZones = ToontownGlobals.HoodHierarchy.get(baseZone)
            for streetZone in streetZones:
                allowedZones.append(streetZone)

        if not config.GetBool('want-spawn-prop', False) or localAvatar.zoneId not in allowedZones:
            return
        X = self.getX()
        Y = self.getY()
        Z = self.getZ()
        H = self.getH()
        propNode = render.find('**/currentPropNode')
        if propNode.isEmpty():
            return
        if globalPropPool.propTypes.get(prop, None) or prop in Variants:
            propNode.show()
            requestedProp = globalPropPool.getProp(prop)
            requestedProp.reparentTo(propNode)
            requestedProp.setPosHpr(X, Y, Z, H, 0, 0)
        return
    ...
    def rainbow(self):
        ...
        seq = Sequence(LerpColorInterval(self, 0.5, green, blue), LerpColorInterval(self, 0.5, red, green), LerpColorInterval(self, 0.5, yellow, red), LerpColorInterval(self, 0.5, orange, yellow), LerpColorInterval(self, 0.5, pink, orange), LerpColorInterval(self, 0.5, blue, pink))
        seq.loop()
