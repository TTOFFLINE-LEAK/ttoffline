...
class LocalAvatar(DistributedAvatar.DistributedAvatar, DistributedSmoothNode.DistributedSmoothNode):
    ...
    def isTeleportAllowed(self):
        return self.teleportAllowed and not self.isTransformed
    ...
    def enableSmartCameraViews(self):
        ...
        if config.GetBool('want-sticky-scroll', False):
            self.accept('wheel_up', self.nextCameraPos, [1])
            self.accept('wheel_down', self.nextCameraPos, [0])
        else:
            self.accept('mouse2', self.enableScroll)
        self.accept('shift-tab', self.nextCameraPos, [0])
        self.accept('control-tab', self.enterFirstPerson)
    ...
    def enableScroll(self):
        self.accept('wheel_up', self.nextCameraPos, [1])
        self.accept('wheel_down', self.nextCameraPos, [0])
        self.accept('mouse2', self.disableScroll)
    ...
    def setWalkSpeedNormal(self):
        self.controlManager.setSpeeds(self.currentSpeed, OTPGlobals.ToonJumpForce, self.currentReverseSpeed, OTPGlobals.ToonRotateSpeed)
    ...
    def enterFirstPerson(self):
        ...
        if self.firstPersonOn:
            ...
            self.firstPersonOn = True
            taskName = self.taskName('updateFPCamera')
            taskMgr.remove(taskName)
            taskMgr.add(self.updateFPCamera, taskName, priority=47)
    ...
    def updateFPCamera(self, task):
        if not self.firstPersonOn:
            return Task.done
        ...
        if base.win.movePointer(0, base.win.getXSize() / 2, base.win.getYSize() / 2):
            self.setH(self.getH() - (x2 - base.win.getXSize() / 2) * 0.01)
        base.win.movePointer(0, int(x2), int(y2))
        camHeight = self.getClampedAvatarHeight()
        self.setCameraSettings((Point3(0.0 - x, 0.7, camHeight - y), Point3(0.0, 1.5, camHeight), Point3(0.0, camHeight, camHeight * 1.33), Point3(0.0, camHeight, camHeight * 0.66), 1))
        return Task.cont
...
def setSpeed(speed=OTPGlobals.ToonForwardSpeed):
    if speed <= 0:
        return 'Cannot have a 0 or negative speed!'
    if speed > 1000:
        return 'Slow down there, Sonic.'
...
def setGravity(gravityValue=ToontownGlobals.GravityValue * 2.0, overrideWarning=False):
    if not base.localAvatar:
        return 'No localAvatar!'
...
def getNumFrames(anim):
    if not base.localAvatar:
        return 'No localAvatar!'
    return str(base.localAvatar.getNumFrames(anim))