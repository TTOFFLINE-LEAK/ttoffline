...
from toontown.toontowngui.PasswordScreen import PasswordScreen
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def __init__(self, serverVersion, launcher=None, playGame=None):
        ...
        self.loginFSM = ClassicFSM('loginFSM', [
         ...
         State('playingGame', self.enterPlayingGame, self.exitPlayingGame, [
          ...
          'periodTimeout',
          'noShards']),
         State('passwordProtected', self.enterPasswordProtected, self.exitPasswordProtected, [
          'noConnection',
          'waitForGameList',
          'reject',
          'failedToConnect',
          'shutdown'])], 'loginOff', 'loginOff')
    ...
    def enterLogin(self):
        ...
        self.accept('csmLoginResponse', self.__handleLoginResponse)
        self.csm.performLogin(self.loginDoneEvent)

    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def __handleLoginResponse(self, passwordProtected):
        if passwordProtected:
            self.loginFSM.request('passwordProtected')
        else:
            self.waitForDatabaseTimeout(requestName='WaitOnCSMLoginResponse')

    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def enterPasswordProtected(self):
        self.passwordScreen = PasswordScreen()
        self.passwordScreen.load()

    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def exitPasswordProtected(self):
        ...
        self.handler = None
        if self.passwordScreen:
            self.passwordScreen.exit()
            self.passwordScreen.unload()
            self.passwordScreen = None
        return
    ...
    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def exitLogin(self):
        if not self.csm.passwordProtected:
            self.loggingInBox.cleanup()
            del self.loggingInBox
            self.cleanupWaitingForDatabase()
            self.ignore(self.loginDoneEvent)
    ...
    def enterFailedToConnect(self, statusCode, statusString):
        ...
        if statusCode == 1403 or statusCode == 1405 or statusCode == 1400:
            ...
            if serverURL == '127.0.0.1':
                message = OTPLocalizer.CRNoConnect % (serverURL, portURL) + OTPLocalizer.CRSpace + OTPLocalizer.CRNoConnectTryAgainLocal
            else:
                message = OTPLocalizer.CRNoConnectTryAgain % (serverURL, portURL)
    ...
    def enterNoConnection(self):
        ...
        reconnect = 1
        moreInfo = 0
        if self.bootedIndex in (152, 155, 156, 157, 127):
            reconnect = 0
        ...
        if reconnect and self.supportsRelogin():
    ...
    def __handleLostConnectionAck(self):
        if self.lostConnectionBox.doneStatus == 'ok' and self.supportsRelogin():
            self.loginFSM.request('connect', [self.serverList])
        else:
            self.loginFSM.request('shutdown')
    ...
    def moveOnFromUberZone(self):
        ...
        if not self.SupportTutorial or base.localAvatar.tutorialAck:
            ...
            if config.GetBool('force-tutorial', 1):
    ...
    def handleGenerateWithRequired(self, di, other=False):
        ...
        for handle, interest in self._interests.items():
            ...
            self.notify.warning('Received generate for %d from %d:%d, not part of any existing interest!' % (
