...
class TutorialManagerAI(DistributedObjectAI):
    ...
    def toonArrived(self):
        ...
        if av.getTutorialAck():
            avFsm = self.avId2fsm.get(avId)
            if avFsm is None:
                self.air.writeServerEvent('suspicious', avId=avId, issue='Possible district reset attempt via code injection!')
                self.notify.warning(('{0} might have attempted to cause a district reset via code injection!').format(avId))
                return
            avFsm.demand('CleanUp')
