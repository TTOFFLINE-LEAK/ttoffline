...
class TownLoader(StateData.StateData):
    ...
    def load(self, zoneId):
        ...
        if base.cr.newsManager:
            if base.cr.newsManager.isHolidayRunning(APRIL_FOOLS_COSTUMES):
                taskMgr.doMethodLater(randint(10, 60), self.pianoDropTask, 'piano-drop-task')
    ...
    def createHood(self, dnaFile, loadStorage=1):
        if loadStorage:
            files = (
             'phase_5/dna/storage_town.jazz', self.townStorageDNAFile)
    ...
    def pianoDropTask(self, task):
        if base.localAvatar.getIsTeleporting():
            ...
            return task.again
        if hasattr(self, 'pianoDropSequence') and self.pianoDropSequence and self.pianoDropSequence.isPlaying():
            task.delayTime = randint(10, 60)
            return task.again
        self.npc = NPCToons.createLocalNPC(random.choice([91915, 14014, 14009, 14006, 14007, 14008, 14011, 14012, 14010, 14035, 14036, 14037, 14038, 14039, 14044, 14045, 14046]))
        ...
        self.pianoDropSequence = Sequence(Func(self.pianoDropSound.start), Parallel(self.piano.scaleInterval(1, (3,
                                                                                                                 3,
