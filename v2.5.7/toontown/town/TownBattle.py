...
class TownBattle(StateData.StateData):
    ...
    def enter(self, event, parentFSMState, bldg=0, creditMultiplier=1, tutorialFlag=0, inSquirtingFlowerEvent=0):
        ...
        base.localAvatar.inventory.setActivateMode('battle', heal=0, bldg=bldg, tutorialFlag=tutorialFlag, inSquirtingFlowerEvent=inSquirtingFlowerEvent)
    ...
    def __handleAttackPanelDone(self, doneStatus):
        ...
        if mode == 'Inventory':
            ...
            if self.track == HEAL_TRACK:
                ...
                elif self.numToons >= 3 and self.numToons <= 8:
    ...
    def adjustCogsAndToons(self, cogs, luredIndices, trappedIndices, toons):
        ...
        for cog in cogs:
            if cog.isBoss:
                self.containsBossCogs = 1
                break

        for cog in cogs:
            maxSuitLevel = max(maxSuitLevel, cog.getActualLevel())
            if cog.getActualLevel() >= 50:
                self.cogFireCosts[cogFireCostIndex] = 69
            else:
        ...
        if resetActivateMode:
            ...
            base.localAvatar.inventory.setBattleCreditMultiplier(self.creditMultiplier)
            base.localAvatar.inventory.setActivateMode('battle', heal=canHeal, trap=canTrap, lure=canLure, bldg=self.bldg, creditLevel=self.creditLevel, tutorialFlag=self.tutorialFlag, inSquirtingFlowerEvent=self.inSquirtingFlowerEvent)
