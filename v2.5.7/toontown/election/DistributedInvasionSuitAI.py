...
class DistributedInvasionSuitAI(DistributedSuitBaseAI, InvasionSuitBase, FSM):
    ...
    def announceGenerate(self):
        if self.spawnPointId == 99:
            ...
            if self.spawnPointId == 100 or self.spawnPointId == 101:
    ...
    def attack(self, who):
        attacks = [
         'clip-on-tie', 'redtape', 'newspaper', 'pink-slip', 'power-tie']
    ...
    def takeDamage(self, hp, bypassFinale=False):
        ...
        if self.finale or not isinstance(self.invasion, DistributedElectionEventAI) and self.spawnPointId == 99:
            if not bypassFinale:
                self.b_setHP(self.currHP + hp)
            return
    ...
    def evidenceStun(self):
        if self.state != 'StunnedByEvidence' and self.state != 'Stunned' and self.state != 'StunnedByEMP':
            self.b_setState('StunnedByEvidence')

    def empStun(self):
        if self.state != 'StunnedByEvidence' and self.state != 'Stunned' and self.state != 'StunnedByEMP':
            self.b_setState('StunnedByEMP')
    ...
    def enterStunnedByEMP(self):
        self.brain.stop()
        self._delay = taskMgr.doMethodLater(SuitTimings.suitStunnedByEMP, self.__unstun, self.uniqueName('stunned'))
