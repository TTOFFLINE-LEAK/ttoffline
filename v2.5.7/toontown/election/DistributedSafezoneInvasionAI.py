...
class DistributedSafezoneInvasionAI(DistributedObjectAI, FSM):
    ...
    def waveWon(self):
        ...
        if self.lastWave:
            ...
            if self.waveNumber in SafezoneInvasionGlobals.SuitWaitWaves:
                self.demand('BeginWave', self.waveNumber + 1)
                self.lastWave = self.waveNumber == len(SafezoneInvasionGlobals.SuitWaves) - 1
            else:
    ...
    def pieHitToon(self, doId):
        ...
        if toon.getHp() == -1:
            toon.setHp(0)
        if toon.pieType == ToontownGlobals.PieEMP:
            toon.takeDamage(1)
        else:
    ...
    def pieHitSuit(self, doId):
        ...
        if suit not in self.suits:
            return
        if toon.pieType == ToontownGlobals.PieEvidence:
            suit.evidenceStun()
        else:
            if toon.pieType == ToontownGlobals.PieEMP:
                suit.takeDamage(random.randrange(65, 175))
                suit.empStun()
            else:
                if toon.pieType == ToontownGlobals.PieSnowball:
                    suit.takeDamage(25)
                else:
