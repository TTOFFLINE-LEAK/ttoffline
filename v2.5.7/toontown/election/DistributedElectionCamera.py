...
class DistributedElectionCamera(DistributedNode):
    ...
    def setState(self, state, ts, x, y, z, h, p, target):
        ...
        if state == 'Move':
            ...
            if state == 'Follow' and target in base.cr.doId2do:
        ...
        self.propInterval = self.prop.hprInterval(1, (0, 0, 0), (360, 0, 0))
        elapsed = globalClockDelta.localElapsedTime(ts)
        movement = Sequence(Parallel(self.camera.posInterval(time, Point3(x, y, z)), self.camBody.hprInterval(time, Vec3(cH, cP, 0)), self.posHprInterval(time, Point3(x, y, z), Vec3(h, p, 0))), Func(self.idleInterval.loop), Func(self.propInterval.loop))
        movement.start()
