...
class DistributedInvasionSuit(DistributedSuitBase, InvasionSuitBase, FSM, DelayDeletable):
    ...
    def makeAttackTrack(self):
        if base.cr.doId2do.get(self.attackTarget, None):
            if base.cr.doId2do[self.attackTarget].getHp() <= 0:
                track = Sequence(Func(self.sayFaceoffTaunt), ActorInterval(self, 'victory', startFrame=0, endFrame=23), Func(self.pingpong, 'victory', fromFrame=24, toFrame=42))
                return track
    ...
    def setInvasionFinale(self, finale):
        if finale and not self.invasionFinale:
            ...
            if not finale and self.invasionFinale:
                pass
            else:
                return
