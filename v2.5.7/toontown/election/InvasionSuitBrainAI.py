...
class InvasionSuitBrainAI(FSM):
    ...
    def __proxemics(self, task):
        for otherSuit in self.suit.invasion.suits:
            ...
            if otherSuit.getActualLevel() < self.suit.getActualLevel():
                ...
                if otherSuit.getActualLevel() == self.suit.getActualLevel() and otherSuit.doId > self.suit.doId:
                    continue
    ...
    def enterAskAgain(self):
        if self.askTask:
            self.askTask.remove()
        self.askTask = taskMgr.doMethodLater(5.0, self.master.requestOrders, self.suit.uniqueName('ask-again'), extraArgs=[self])
