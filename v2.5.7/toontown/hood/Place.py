...
class Place(StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def __handleBook(self, serverList=None):
        base.localAvatar.stopSleepWatch()
        base.localAvatar.book.exit()
    ...
    def __handleBookExitTeleport(self, requestStatus):
        ...
        if self.serverList:
            base.cr.loginFSM.request('loginOff')
            base.cr.loginFSM.request('connect', [self.serverList])
    ...
    def handleEnterTunnel(self, requestStatus, collEntry):
        ...
        if requestStatus['zoneId'] == 19000 and base.localAvatar.cogParts[4] != CogDisguiseGlobals.PartsPerSuitBitmasks[4]:
            ...
            del dummyNP
            return
        if requestStatus['zoneId'] == ToontownGlobals.OldDaisyGardens and base.localAvatar.zoneId == ToontownGlobals.OldOakStreet + 1:
            return
    ...
    def enterDoorIn(self, requestStatus):
        ...
        if requestStatus['doorDoId'] in base.cr.doId2do:
            self.setDoor(base.cr.doId2do[requestStatus['doorDoId']])
        else:
            self.acceptOnce('generate-%d' % requestStatus['doorDoId'], self.setDoor)
    ...
    def enterTeleportOut(self, requestStatus, callback):
        base.localAvatar.setIsTeleporting(True)
        if base.localAvatar.zoneId != ToontownGlobals.OldDaisyGardens:
            base.localAvatar.laffMeter.start()
        if requestStatus['zoneId'] == ToontownGlobals.BossbotHQ and base.localAvatar.zoneId == ToontownGlobals.SellbotHQ:
    ...
    def exitTeleportOut(self):
        ...
        base.localAvatar.stopQuestMap()
        base.localAvatar.obscureMoveFurnitureButton(-1)
        base.localAvatar.setIsTeleporting(False)

    def enterDied(self, requestStatus, callback=None):
        if self.zoneId % 1000 == 0 and base.config.GetBool('want-doomsday', False):
