...
class Hood(StateData.StateData):
    ...
    def getHoodText(self, zoneId):
        ...
        if self.id != Tutorial:
            ...
            if zoneId == ToontownGlobals.OldDaisyGardens:
                hoodText = ''
            elif zoneId in ToontownGlobals.ShortWorkZones:
                hoodText = TTLocalizer.GlobalStreetNames.get(zoneId)[2]
            elif streetName:
                hoodText = hoodText + '\n' + streetName[(-1)]
        return hoodText
    ...
    def load(self):
        ...
        if newsManager:
            ...
            if (base.cr.newsManager.isHolidayRunning(TRICK_OR_TREAT) or base.cr.newsManager.isHolidayRunning(HALLOWEEN_PROPS) or base.cr.newsManager.isHolidayRunning(HALLOWEEN)) and self.spookySkyFile:
