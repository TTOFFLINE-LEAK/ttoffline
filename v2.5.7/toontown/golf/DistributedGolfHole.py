...
class DistributedGolfHole(DistributedPhysicsWorld.DistributedPhysicsWorld, FSM, GolfHoleBase.GolfHoleBase):
    ...
    def updateWarning(self):
        ...
        if self.localMissedSwings == GolfGlobals.KICKOUT_SWINGS - 1:
            ...
            if self.localMissedSwings >= GolfGlobals.KICKOUT_SWINGS:
    ...
    def enterPlayback(self):
        ...
        if self.ballWillGoInHole:
            ...
            if self.ballFirstTouchedHoleTime:
                crowdIval = Sequence()
                buildupLength = crowdBuildupSfx.length()
    ...
    def handleBallGoingInHole(self):
        ...
        if strokes == 1:
            ...
            if diff in TTLocalizer.GolfShotDesc:
                if self.ballWillGoInHole:
        ...
        if strokes == 1:
            ...
            if self.hasCurGolferReachedMaxSwing and not self.ballWillGoInHole:
    ...
    def postStep(self):
        if self.currentGolferActive:
            ...
            if self.inPlayBack == 2:
                ...
                if self.state == 'Playback' and self.inPlayBack == 0:
                    self.request('Wait')
    ...
    def setHoleId(self, holeId):
        self.holeId = holeId
        if self.holeId == 36:
            self.pumpk = Actor('phase_6/models/golf/pump_zero', {'dance': 'phase_6/models/golf/pump_dance'})
            self.pumpk.reparentTo(render)
            self.pumpk.loop('dance')
            self.pumpk1 = Actor('phase_6/models/golf/pump_zero_1', {'dance2': 'phase_6/models/golf/pump_dance_1'})
            self.pumpk1.reparentTo(render)
            self.pumpk1.setPos(95.822, 15.2, -1.5)
            self.pumpk1.setHpr(-90, 0, 0)
            self.pumpk1.loop('dance2')
            self.music = base.loader.loadMusic('phase_12/audio/bgm/Bossbot_PlayGolf_v1.ogg')
        else:
            if self.holeId == 37:
                self.pumpk = Actor('phase_6/models/golf/pump_zero', {'dance': 'phase_6/models/golf/pump_dance'})
                self.pumpk.reparentTo(render)
                ...
                self.pumpk1 = Actor('phase_6/models/golf/pump_zero_1', {'dance2': 'phase_6/models/golf/pump_dance_1'})
                self.pumpk1.reparentTo(render)
                self.pumpk1.setPos(90.822, -15.2, -1.5)
                self.pumpk1.setHpr(-90, 0, 0)
                self.pumpk1.loop('dance2')
                self.music = base.loader.loadMusic('phase_12/audio/bgm/Bossbot_PlayGolf_v1.ogg')
            else:
    ...
    def doFlyOverMovie(self, avId):
        ...
        try:
            self.flyOverActor = Actor(camModelFullPath, {'camera': cameraAnimFullPath})
            self.flyOverActor.setBlend(frameBlend=config.GetBool('interpolate-animations', True))
    ...
    def handleBallHitNonGrass(self, c0, c1):
        ...
        if GolfGlobals.HARD_COLLIDE_ID in [c0, c1]:
            ...
            if GolfGlobals.MOVER_COLLIDE_ID in [c0, c1]:
                if not self.bumpMoverSfx.status() == self.bumpMoverSfx.PLAYING:
                    base.playSfx(self.bumpMoverSfx)
                    self.lastBumpSfxPos = golfBallPos
            else:
                if GolfGlobals.WINDMILL_BASE_COLLIDE_ID in [c0, c1]:
