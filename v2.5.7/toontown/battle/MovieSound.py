...
def __getSuitTrack(sound, lastSoundThatHit, delay, hitCount, targets, totalDamage, hpbonus, toon, npcs):
    ...
    for target in targets:
        ...
        if totalDamage > 0 and sound == lastSoundThatHit:
            ...
            if hitCount == 1:
                if suit.isBoss:
                    suitTrack.append(Parallel(ActorInterval(suit.suitBoss, 'Fb_squirt-small'), MovieUtil.createSuitStunInterval(suit, 0.5, 1.8)))
                else:
                    suitTrack.append(Parallel(ActorInterval(suit, 'squirt-small-react'), MovieUtil.createSuitStunInterval(suit, 0.5, 1.8)))
            else:
                if suit.isBoss:
                    suitTrack.append(ActorInterval(suit.suitBoss, 'Fb_squirt-small'))
                else:
...
def __createToonInterval(sound, delay, toon, operaInstrument=None):
    ...
    if DISTANCE_TO_WALK_BACK and hasLuredSuits and not isNPC:
        if toon.isDisguised:
            retval.append(Parallel(ActorInterval(toon.suit, 'walk', startTime=1, duration=TIME_TO_WALK_BACK, endTime=0.0001), LerpPosInterval(toon, TIME_TO_WALK_BACK, newPos, other=battle)))
    ...
    if toon.isDisguised:
        if toon.suit.style.body == 'c':
            retval.append(Func(toon.suit.pose, anim, 0))
    if operaInstrument:
        ...
        I1 = 2.8
        if toon.isDisguised:
            retval.append(Wait(waitTime))
            retval.append(ActorInterval(toon.suit, anim, playRate=1.0, startTime=0.0, endTime=operaNo))
            ...
            retval.append(ActorInterval(toon.suit, anim, playRate=1.0, startTime=operaNo))
        else:
        ...
        if toon.isDisguised:
            retval.append(Wait(waitTime))
            retval.append(ActorInterval(toon.suit, anim))
        else:
...
def __doBikehorn(sound, delay, toon, targets, level):
    ...
    def setInstrumentStats(instrument=instrument, instrument2=instrument2, megaphone=megaphone):
        if toon.isDisguised:
            if toon.suit.style.body != 'a':
                coolX = 0
                coolY = 2
                coolZ = 0
                coolH, coolP, coolR = megaphone.getHpr()
                coolH = coolH - 27
                coolP = coolP - 70
                coolR = coolR - 90
            else:
                ...
                coolR = 0
            iPos = (
             -0.2 + coolX, -1 + coolY, 0.7 + coolZ)
            iHpr = (182.5 + coolH, 30 + coolP, 0 + coolR)
        else:
            iPos = (-1.1, -1.4, 0.1)
            iHpr = (145, 0, 0)
        instrument.setPos(iPos)
        instrument.setHpr(iHpr)
        instrument.setScale(instrMin)
        instrument2.setPos(iPos)
        instrument2.setHpr(iHpr)
...
def __doFoghorn(sound, delay, toon, targets, level):
    ...
    def setInstrumentStats(instrument=instrument, instrument2=instrument2, megaphone=megaphone, justgetthedamnvalues=0):
        if toon.isDisguised:
            if toon.suit.style.body != 'a':
                coolX = 0
                coolY = 2
    ...
    if toon.isDisguised:
        coolH, coolP, coolR = setInstrumentStats(justgetthedamnvalues=1)
        spinInstr1 = LerpHprInterval(instrument, duration=1.5, startHpr=Vec3(coolH, coolP, 0), hpr=Vec3(coolH, coolP, coolR + 90), blendType='easeInOut')
        spinInstr2 = LerpHprInterval(instrument2, duration=1.5, startHpr=Vec3(coolH, coolP, 0), hpr=Vec3(coolH, coolP, coolR + 90), blendType='easeInOut')
    else:
