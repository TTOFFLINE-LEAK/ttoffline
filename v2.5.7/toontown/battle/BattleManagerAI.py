import DistributedBattleAI, DistributedBattleSquirtingFlowerAI
...
class BattleManagerAI:
    ...
    def newBattle(self, cellId, zoneId, pos, suit, toonId, finishCallback=None, maxSuits=8, interactivePropTrackBonus=-1):
        if self.cellId2battle.has_key(cellId):
            ...
            if zoneId == ToontownGlobals.OldOakStreet + 14 and self.air.currentEpisode == 'squirting_flower':
                battle = DistributedBattleSquirtingFlowerAI.DistributedBattleSquirtingFlowerAI(self.air, self, pos, suit, toonId, zoneId, finishCallback, maxSuits, inSquirtingFlowerEvent=1, interactivePropTrackBonus=interactivePropTrackBonus)
            else:
