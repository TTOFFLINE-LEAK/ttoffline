...
def __runToHealSpot(heal):
    ...
    if toon.isDisguised:
        runAnimI = Func(toon.suit.loop, 'run')
    else:
...
def __healTickle(heal, hasInteractivePropHealBonus):
    ...
    if target.isDisguised:
        mtrack = Parallel(featherTrack, manim, __getSoundTrack(level, 1, node=toon), Sequence(Wait(tHeal), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus), ActorInterval(target.suit, 'squirt-small-react'), Func(target.suit.loop, 'neutral')))
    else:
...
def __healJoke(heal, hasInteractivePropHealBonus):
    ...
    if miniserver or npcId != 0:
        ...
        if toon.isDisguised:
            ...
            tracks.append(Sequence(Func(toon.loop, 'neutral'), Wait(tDoSoundAnimation), ActorInterval(toon, 'sound')))
    else:
        if toon.isDisguised:
            pass
        else:
            tracks.append(Sequence(ActorInterval(targets, 'think', playRate=0.638, startFrame=0, endFrame=46), ActorInterval(targets, 'think', startFrame=46), Func(toon.loop, 'neutral')))
    soundTrack = __getSoundTrack(level, 2.0, node=toon)
    tracks.append(soundTrack)
    joke = HealJokes.toonHealJokes[jokeIndex]
    tracks.append(Sequence(Wait(tSpeakSetup), Func(toon.setChatAbsolute, joke[0], msgType)))
    tracks.append(Sequence(Wait(tSpeakPunchline), Func(toon.setChatAbsolute, joke[1], msgType)))
    ...
    if miniserver:
        ...
        hp = heal['target']['hp']
        reactTrack.append(Func(__healToon, targets, hp, ineffective, hasInteractivePropHealBonus))
...
def __healSmooch(heal, hasInteractivePropHealBonus):
    ...
    dScale = 0.5
    lipstickTrack = Sequence(Func(MovieUtil.showProps, lipsticks, rightHands, pos, hpr), MovieUtil.getScaleIntervals(lipsticks, dScale, MovieUtil.PNT3_NEARZERO, MovieUtil.PNT3_ONE), Wait(toon.getDuration('smooch') - 2.0 * dScale), MovieUtil.getScaleIntervals(lipsticks, dScale, MovieUtil.PNT3_ONE, MovieUtil.PNT3_NEARZERO), Func(MovieUtil.removeProps, lipsticks))
    ...
    if miniserver:
        ...
        lipsTrack = Sequence(Wait(tLips), Func(MovieUtil.showProp, lips, render, getLipPos), Func(lips.setBillboardPointWorld), LerpScaleInterval(lips, dScale, Point3(3, 3, 3), startScale=MovieUtil.PNT3_NEARZERO), Wait(tThrow - tLips - dScale), LerpPosInterval(lips, dThrow / 2, Point3(0, 0, target.getHeight()), other=battle, blendType='easeOut'), LerpPosInterval(lips, dThrow / 2, Point3(target.getPos() + Point3(0, 0, target.getHeight())), blendType='easeIn'), Func(MovieUtil.removeProp, lips))
    delay = tThrow + dThrow
    ...
    if toon.isDisguised:
        ...
        if toon.suit.style.body == 'c':
            rate = 1.0
            start = 50
        jacob = Sequence(ActorInterval(toon.suit, 'finger-wag', playRate=0.5, endFrame=16), Wait(3), ActorInterval(toon.suit, 'throw-paper', playRate=rate, startFrame=start))
    paul = ActorInterval(target, 'conked')
    if target.isDisguised:
        paul = ActorInterval(target.suit, 'pie-small-react')
    if miniserver:
        jacob.append(Sequence(*__returnToBase(heal)))
        mtrack = Parallel(lipstickTrack, lipsTrack, __getSoundTrack(level, 2, node=toon), jacob, Sequence(Wait(delay), paul), Sequence(Wait(delay), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus)))
    else:
        mtrack = Parallel(lipstickTrack, lipsTrack, __getSoundTrack(level, 2, node=toon), jacob, Sequence(Wait(delay), paul), Sequence(Wait(delay), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus)))
    track.append(mtrack)
    track.append(Func(target.clearChat))
...
def __healDance(heal, hasInteractivePropHealBonus):
    ...
    if miniserver:
        ...
        hp = heal['target']['hp']
        reactIval = Func(__healToon, targets, hp, ineffective, hasInteractivePropHealBonus)
        if first:
            targetTrack.append(Wait(delay))
            first = 0
        targetTrack.append(reactIval)
    ...
    if toon.isDisguised:
        cogdance = ActorInterval(toon.suit, 'song-and-dance')
        if toon.suit.style.body != 'a':
            cogdance = ActorInterval(toon.suit, 'victory', playRate=1.45139)
        mtrack = Parallel(propTrack, cogdance, __getSoundTrack(level, 0.2, duration=6.4, node=toon), targetTrack)
    track.append(Func(toon.loop, 'neutral'))
    ...
    if npcId != 0:
        ...
        if not miniserver:
            track.append(Func(toon.loop, 'neutral'))
            track.append(Func(targets.clearChat))
        else:
            track.append(__returnToBase(heal))
    if miniserver:
        for target in targets:
...
def __healSprinkle(heal, hasInteractivePropHealBonus):
    ...
    if not miniserver:
        sprayEffect.setP(90)
        explodeEffect.setR(-115)
        wallEffect.setH(90)
    else:
        if toon.isDisguised:
            sprayEffect.setH(90)
            explodeEffect.setH(90)
            wallEffect.setH(90)

    def face90(toon=toon, target=target, chag=None):
        if miniserver:
            ...
            if not toon.isDisguised:
                ...
                toon.headsUp(render, target.getPos())

    delay = 2.5
    boob = Sequence(Func(face90), ActorInterval(toon, 'sprinkle-dust'))
    if toon.isDisguised:
        boob = Sequence(Func(face90), ActorInterval(toon.suit, 'finger-wag', endFrame=54), ActorInterval(toon.suit, 'throw-paper', startFrame=85, endFrame=60), ActorInterval(toon.suit, 'throw-paper', startFrame=60))
        if toon.suit.style.body == 'c':
            boob = Sequence(ActorInterval(toon.suit, 'finger-wag', endFrame=54), ActorInterval(toon.suit, 'throw-paper', startFrame=70, endFrame=45), ActorInterval(toon.suit, 'throw-paper', startFrame=40))
    mtrack = Parallel(__getPartTrack(sprayEffect, 1.5, 0.5, [sprayEffect, toon, 0]), __getPartTrack(dropEffect, 1.9, 2.0, [dropEffect, target, 0]), __getPartTrack(explodeEffect, 2.7, 1.0, [explodeEffect, toon, 0]), __getPartTrack(poofEffect, 3.4, 1.0, [poofEffect, target, 0]), __getPartTrack(wallEffect, 4.05, 1.2, [wallEffect, toon, 0]), __getSoundTrack(level, 2, duration=4.1, node=toon), boob, Sequence(Wait(delay), Func(__healToon, target, hp, ineffective, hasInteractivePropHealBonus)))
    track.append(mtrack)
    if miniserver:
        track.append(__returnToBase(heal))
    else:
