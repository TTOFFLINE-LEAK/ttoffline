...
class DistributedBattleBaseAI(DistributedObjectAI.DistributedObjectAI, BattleBase):
    ...
    def __init__(self, air, zoneId, finishCallback=None, maxSuits=4, bossBattle=0, tutorialFlag=0, inSquirtingFlowerEvent=0, interactivePropTrackBonus=-1, maxToons=4):
        DistributedObjectAI.DistributedObjectAI.__init__(self, air)
        self.serialNum = 0
        self.zoneId = zoneId
        self.maxSuits = maxSuits
        self.maxToons = maxToons
        ...
        self.toonMerits = {}
        self.toonParts = {}
        self.battleCalc = BattleCalculatorAI(self, tutorialFlag, inSquirtingFlowerEvent)
    ...
    def getMovie(self):
        ...
        for t in self.activeToons:
            if self.toonAttacks.has_key(t):
                ...
                if (track == NO_ATTACK or attackAffectsGroup(track, ta[TOON_LVL_COL])) and track != NPCSOS and track != PETSOS:
                    ...
                    if track == SOS or track == NPCSOS or track == PETSOS:
        ...
        for i in range(8 - len(self.activeToons)):
            p = p + getToonAttack(-1)
    ...
    def getBattleExperience(self):
        returnValue = BattleExperienceAI.getBattleExperience(8, self.activeToons, self.toonExp, self.battleCalc.toonSkillPtsGained, self.toonOrigQuests, self.toonItems, self.toonOrigMerits, self.toonMerits, self.toonParts, self.suitsKilled, self.helpfulToons)
        return returnValue
    ...
    def enterWaitForInput(self):
        ...
        if len(self.toons) >= 5:
            adjustment = 10
        if not (self.tutorialFlag or self.inSquirtingFlowerEvent):
            self.timer.startCallback(SERVER_INPUT_TIMEOUT + adjustment, self.__serverTimedOut)
        self.npcAttacks = {}
