...
hitSoundFiles = ('AA_drop_flowerpot.ogg', 'AA_drop_sandbag.ogg', 'AA_drop_anvil.ogg',
                 ...
                 'AA_drop_boat.ogg')
briefHitSoundFiles = ('AA_drop_flowerpot.ogg', 'AA_drop_sandbag.ogg', 'AA_drop_anvil.ogg',
                      'AA_drop_bigweight_short.ogg', 'AA_drop_safe_short.ogg', 'AA_drop_piano_short.ogg',
                      'AA_drop_boat.ogg')
...
def __getSoundTrack(level, hitSuit, suit, node=None):
    if hitSuit:
        if suit.isBoss:
            soundEffect = globalBattleSoundCache.getSound(briefHitSoundFiles[level])
        else:
...
def __dropObject(drop, delay, objName, level, alreadyDodged, alreadyTeased, npcs, target, npcDrops):
    ...
    scaleMulti = 1
    if level == 6:
        if len(drop['target']) >= 5:
            scaleMulti = 1.6
        if suit.isBoss:
            scaleMulti = len(drop['target']) * 1.2
    node = object.node()
    node.setBounds(OmniBoundingVolume())
    node.setFinal(1)
    soundTrack = __getSoundTrack(level, hitSuit, suit, toon)
    toonTrack = Sequence()
    if repeatNPC == 0:
    ...
    def posObject(object, suit, level, majorObject, miss, battle=battle):
        ...
        if suit.isBoss and level == 6 and len(drop['target']) > 1:
            if len(drop['target']) == 2:
                offset = -10
            else:
                if len(drop['target']) == 3:
                    offset = -1
                else:
                    if len(drop['target']) == 4:
                        offset = -15
            object.setX(object.getX(battle) + offset)
        if suit.isBoss and level != 6:
            object.setZ(object.getZ(battle) + 10)
    ...
    if hp > 0 or level == 1 or level == 2:
        if hasattr(object, 'getAnimControls'):
            if suit.isBoss and majorObject:
                animProp = ActorInterval(object, objName, endFrame=30)
            else:
            ...
            startingScale = objStartingScales[level] * scaleMulti
...
def __createSuitTrack(drop, delay, level, alreadyDodged, alreadyTeased, target, npcs):
    ...
    if hp > 0:
        ...
        if majorObject:
            ...
            anim = 'drop-react'
        if suit.isBoss:
            suitReact = ActorInterval(suit.suitBoss, 'Fb_anvil-drop')
        else:
