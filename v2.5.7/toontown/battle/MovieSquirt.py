...
def __getSuitTrack(suit, tContact, tDodge, hp, hpbonus, kbbonus, anim, died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun=0.5, afterStun=1.8, geyser=0, uberRepeat=0, revived=0):
    if hp > 0:
        if suit.isBoss:
            if anim == 'soak':
                bossAnim = 'Fb_soak'
            elif anim == 'squirt-small-react':
                bossAnim = 'Fb_squirt-small'
            else:
                bossAnim = 'Fb_downHit'
        suitTrack = Sequence()
            ...
            if geyser:
                ...
                if suitType == 'a':
                    ...
                    if suitType == 'b':
                        ...
                        endFlailFrame = 15
                if suit.isBoss:
                    sival = Sequence(ActorInterval(suit.suitBoss, 'Fb_downHit', playRate=0.5, startFrame=0, endFrame=startFlailFrame - 1), Func(suit.suitBoss.pingpong, 'Fb_downHit', fromFrame=startFlailFrame, toFrame=endFlailFrame), Wait(0.5), ActorInterval(suit.suitBoss, 'Fb_downHit', playRate=1.0, startFrame=endFlailFrame))
                else:
                ...
                if fShowStun == 1:
                    if suit.isBoss:
                        sival = Parallel(ActorInterval(suit.suitBoss, bossAnim), MovieUtil.createSuitStunInterval(suit, beforeStun, afterStun))
                    else:
                        sival = Parallel(ActorInterval(suit, anim), MovieUtil.createSuitStunInterval(suit, beforeStun, afterStun))
                else:
                    if suit.isBoss:
                        sival = ActorInterval(suit.suitBoss, bossAnim)
                    else:
                        sival = ActorInterval(suit, anim)
        ...
        if died != 0:
            ...
            if suit.isBoss:
                suitTrack.append(Func(suit.suitBoss.loop, 'Fb_downNeutral'))
            else:
...
def __doFlower(squirt, delay, fShowStun):
    ...
    if toon.isDisguised:
        flower_joint0 = toon.suit.getHealthBar()
        flower_joint1 = toon.suit.getHealthBar()
        ...
        flowerTrack = Sequence(Wait(tFlowerFirstAppears), Func(flower.reparentTo, flower_joint0), Func(flower.setP, 180), Func(flower.setY, 0.2), LerpScaleInterval(flower, dFlowerScaleTime, flower.getScale(), startScale=MovieUtil.PNT3_NEARZERO), Wait(tTotalFlowerToonAnimationTime - dFlowerScaleTime - tFlowerFirstAppears))
    else:
...
def __doWaterGlass(squirt, delay, fShowStun):
    ...
    battle = squirt['battle']
    if toon.isDisguised:
        pen = globalPropPool.getProp('bluepen')

        def getPenTip(pen=pen):
            tip = pen.find('**/joint_toSpray')
            return tip.getPos(render)

        hitPoint = lambda suit=suit: __suitTargetPoint(suit)
        hitSprayTrack = MovieUtil.getSprayTrack(battle, VBase4(WaterSprayColor), getPenTip, hitPoint, 0.2, 0.2, 0.2, horizScale=0.1, vertScale=0.1)
        toonTrack = Sequence(ActorInterval(toon.suit, 'pen-squirt'), Func(toon.suit.loop, 'neutral'))
        propTrack = Sequence(Wait(0.01), Func(MovieUtil.showProp, pen, toon.suit.getRightHand(), MovieUtil.PNT3_ZERO), LerpScaleInterval(pen, 0.5, Point3(1.5, 1.5, 1.5)), Wait(1.05))
        ...
        if hp > 0:
            scale = sprayScales[level]
            splash = globalPropPool.getProp('splash-from-splat')
            splash.setColor(WaterSprayColor)
            splash.setScale(0.15)
            splashTrack = Sequence(Func(battle.movie.needRestoreRenderProp, splash), Wait(1.65), Func(prepSplash, splash, __suitTargetPoint(suit)), ActorInterval(splash, 'splash-from-splat'), Func(MovieUtil.removeProp, splash), Func(battle.movie.clearRenderProp, splash))
        soundTrack = getSoundTrack('SA_fountain_pen.ogg', delay=1.6, node=toon)
        tracks = Parallel(toonTrack, propTrack, soundTrack, splashTrack)
        if hp > 0 or delay <= 0:
            tracks.append(__getSuitTrack(suit, tContact, tSuitDodges, hp, hpbonus, kbbonus, 'squirt-small-react', died, leftSuits, rightSuits, battle, toon, fShowStun, revived=revived))
    else:
        suitPos = suit.getPos(battle)
...
def __doWaterGun(squirt, delay, fShowStun):
    ...
    tracks = Parallel()
    if toon.isDisguised:
        toonTrack = Sequence(Func(toon.headsUp, battle, suitPos), ActorInterval(toon.suit, 'finger-wag', playRate=0.5, endFrame=16), Wait(3), ActorInterval(toon.suit, 'finger-wag', startFrame=16, endFrame=0), Func(toon.suit.loop, 'neutral'), Func(toon.setHpr, battle, origHpr))
    else:
    ...
    pistolHpr = VBase3(85.6, -4.44 - ccc, 94.43)
...
def __doSeltzerBottle(squirt, delay, fShowStun):
    ...
    if toon.isDisguised:
        if toon.suit.style.body == 'c':
            ...
            if toon.suit.style.body == 'a':
                ...
                iHpr = (100, -50, 0)
                frame = 19
                sec = 2.416666666666667
            else:
                iPos = (0.1, -0.5, -1.7)
                iHpr = (90, -45, 0)
                frame = 24
                sec = 2
        toonTrack = Sequence(Func(toon.headsUp, battle, suitPos), ActorInterval(toon.suit, 'phone', endFrame=frame), Wait(sec), ActorInterval(toon.suit, 'phone', startFrame=frame, endFrame=0), Func(toon.suit.loop, 'neutral'), Func(toon.setHpr, battle, origHpr))
    else:
    ...
    if toon.isDisguised:
        bottle.setPosHpr(Point3(iPos), Point3(iHpr))
        hands = toon.suit.getLeftHands()
    else:
        hands = toon.getRightHands()
    targetPoint = lambda suit=suit: __suitTargetPoint(suit)
...
def __doFireHose(squirt, delay, fShowStun):
    ...
    if not toon.isDisguised:
        ...
        if toon.suit.style.body == 'c':
            ...
            playRate = 1.2778
        toonTrack = Sequence(ActorInterval(toon.suit, 'throw-paper', playRate=playRate), Func(toon.suit.loop, 'neutral'), Func(toon.setHpr, battle, origHpr))
    tracks.append(toonTrack)
    ...
    if not toon.isDisguised:
        ...
        hose = globalPropPool.getProp('hydrant-cog')
        hose.reparentTo(toon.suit.getRightHand())
        hoseValue = False
    targetPoint = lambda suit=suit: __suitTargetPoint(suit)

    def getSprayStartPos(hose=hose, toon=toon, targetPoint=targetPoint, hoseValue=hoseValue):
        toon.update(0)
        if hose.isEmpty() == 1:
    ...
    def setHydrantHpr(hose=hose):
        hose.lookAt(suit)
        hose.setHpr(hose.getH(battle) + 180, 0, 0)
    ...
    if not toon.isDisguised:
        ...
        propTrack = Sequence(Wait(tSprayWait), Func(hose.wrtReparentTo, battle), Func(hose.setScale, 1), Func(hose.lookAt, battle.getPos() + Point3(toon.getX(battle), 0, 0)), Parallel(LerpPosInterval(hose, tSprayDelay - tSprayWait, (toon.getX(battle), 0, 0), other=battle), LerpHprInterval(hose, 0.2, (-720, 0, 0), other=battle)), Func(setHydrantHpr), Wait(dSprayHold + 0.25), LerpScaleInterval(hose, 1, (0, 0, 0)), Func(battle.movie.clearRenderProp, hose))
...
def __doStormCloud(squirt, delay, fShowStun):
    ...
    if suit.isBoss:
        scaleUpPoint = Point3(8, 8, 8)
    else:
        scaleUpPoint = Point3(3, 3, 3)
    rainEffects = [
     rainEffect, rainEffect2, rainEffect3]
    rainDelay = 1
    effectDelay = 0.3
    if hp > 0:
        cloudHold = 4.7
