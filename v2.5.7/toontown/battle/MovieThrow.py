...
def __throwPie(throw, delay, hitCount):
    ...
    if toon.isDisguised:
        if toon.suit.style.body == 'c':
            ...
            toonTrack.append(ActorInterval(toon.suit, 'throw-object', playRate=1.15))
    ...
    if hitSuit:
        if toon.isDisguised:
            pieFly = Sequence(Func(pie.show), LerpPosInterval(pie, tPieHitsSuit - tPieLeavesHand, pos=MovieUtil.avatarFacePoint(suit, other=battle), name=pieFlyTaskName, other=battle))
        else:
    ...
    if hitSuit:
        ...
        if kbbonus > 0:
            ...
            if suit.isBoss:
                animTrack.append(ActorInterval(suit.suitBoss, 'Fb_downHit'))
            else:
                if suitType == 'a':
            ...
            if hitCount == 1:
                if suit.isBoss:
                    sival = Parallel(ActorInterval(suit.suitBoss, 'Fb_downHit'), MovieUtil.createSuitStunInterval(suit, 0.3, 1.3))
                else:
        ...
        if revived != 0:
            ...
            if died != 0:
                ...
                if suit.isBoss:
                    suitResponseTrack.append(Func(suit.suitBoss.loop, 'Fb_downNeutral'))
                else:
...
def __createWeddingCakeFlight(throw, groupHitDict, pie, pies):
    ...
    cakePartDivisions[6] = [[cakeParts[0]],
     ...
     [
      cakeParts[5]]]
    cakePartDivisions[7] = [[cakeParts[0]],
     [
      cakeParts[1]],
     [
      cakeParts[2]],
     ...
     [
      cakeParts[6]]]
    cakePartDivisions[8] = [[cakeParts[0]],
     [
      cakeParts[1]],
     [
      cakeParts[2]],
        ...
        if hitSuit:
            ...
            if revived != 0:
                ...
                if died != 0:
                    ...
                    if suit.isBoss:
                        singleSuitResponseTrack.append(Func(suit.suitBoss.loop, 'Fb_downNeutral'))
                    else:
