...
def getTotalMerits(toon, index):
    ...
    if index == 4:
        return 0
    if toon.cogTypes[index] > 7:
        toon.cogTypes[index] = 7
    cogIndex = toon.cogTypes[index] + SuitDNA.suitsPerDept * index
    meritIndex = toon.cogTypes[index] + SuitDNA.normalSuits * index
    ...
    cogLevel = max(min(cogLevel, len(MeritsPerLevel[meritIndex]) - 1), 0)
    return MeritsPerLevel[meritIndex][cogLevel]
