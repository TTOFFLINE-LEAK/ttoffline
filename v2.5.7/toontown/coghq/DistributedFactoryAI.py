...
from toontown.coghq import DistributedBattleFactoryAI, DistributedBattleSwagtoryAI
...
class DistributedFactoryAI(DistributedLevelAI.DistributedLevelAI, FactoryBase.FactoryBase):
    ...
    def generate(self):
        ...
        if self.getFactoryId() == ToontownGlobals.SwagFactoryInt:
            self.planner = LevelSuitPlannerAI.LevelSuitPlannerAI(self.air, self, DistributedFactorySuitAI.DistributedFactorySuitAI, DistributedBattleSwagtoryAI.DistributedBattleSwagtoryAI, cogSpecModule.CogData, cogSpecModule.ReserveCogData, cogSpecModule.BattleCells)
        else:
    ...
    def getTaskZoneId(self):
        if self.factoryId == ToontownGlobals.SwagFactoryInt:
            return ToontownGlobals.SellbotFactoryInt
    ...
    def setVictors(self, victorIds):
        ...
        for toon in activeVictors:
            if self.factoryId == ToontownGlobals.SwagFactoryInt:
                simbase.air.questManager.toonDefeatedFactory(toon, ToontownGlobals.SellbotFactoryInt, activeVictors)
            else:
