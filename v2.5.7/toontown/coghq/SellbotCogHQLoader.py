...
import FactoryExterior, FactoryInterior, SwagtoryInterior, SellbotHQExterior, SellbotHQWestWing, SellbotHQBossBattle
...
class SellbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def __init__(self, hood, parentFSMState, doneEvent):
        ...
        self.fsm.addState(State.State('swagtoryInterior', self.enterSwagtoryInterior, self.exitSwagtoryInterior, ['quietZone', 'factoryExterior']))
        for stateName in ['quietZone']:
            state = self.fsm.getStateNamed(stateName)
            state.addTransition('factoryInterior')
            state.addTransition('swagtoryInterior')
        ...
        self.westWingModelPath = 'phase_9/models/cogHQ/westWing'
        self.resistanceHallModelPath = 'phase_14/models/modules/resistance_hall_worn'
        self.geom = None
        self.factGeom = None
        self.spotlightSeqs = []
        return
            ...
            for x in xrange(5):
                ...
                if x == 1:
                    ...
                    if x == 5:
                        startTime == 5.9
                        midTime == 7.9
                spotlightSeq = Sequence(spotlight.hprInterval(startTime, (0, 0, -6.9), blendType='easeInOut'), spotlight.hprInterval(midTime, (0,
                                                                                                                                               0,
                                                                                                                                               6.9), blendType='easeInOut'), spotlight.hprInterval(endTime, (0,
                                                                                                                                                                                                             0,
    ...
    def enterFactoryExterior(self, requestStatus):
        if requestStatus['zoneId'] == ToontownGlobals.SellbotWestWing:
            self.placeClass = SellbotHQWestWing.SellbotHQWestWing
        else:
    ...
    def enterSwagtoryInterior(self, requestStatus):
        self.placeClass = SwagtoryInterior.SwagtoryInterior
        self.enterPlace(requestStatus)

    def exitSwagtoryInterior(self):
        self.exitPlace()
        self.placeClass = None
        return
