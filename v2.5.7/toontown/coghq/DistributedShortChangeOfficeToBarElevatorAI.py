...
class DistributedShortChangeOfficeToBarElevatorAI(DistributedElevatorExtAI.DistributedElevatorExtAI):
    ...
    def elevatorClosed(self):
        ...
        if numPlayers > 0:
            ...
            for seatIndex in range(len(self.seats)):
                ...
                if avId:
                    self.sendUpdateToAvatarId(avId, 'setDestinationZone', [ToontownGlobals.CashbotBar])
    ...
    def sendAvatarsToDestination(self, avIdList):
        if len(avIdList) > 0:
            for avId in avIdList:
                if avId:
                    self.sendUpdateToAvatarId(avId, 'setDestinationZoneForce', [ToontownGlobals.CashbotBar])