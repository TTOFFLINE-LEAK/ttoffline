...
class DistributedBanquetTable(DistributedObject.DistributedObject, FSM.FSM, BanquetTableBase.BanquetTableBase):
    ...
    def disable(self):
        ...
        taskMgr.remove(self.nametagName)
    ...
    def announceGenerate(self):
        ...
        self.nametagName = self.uniqueName('watchControls')
    ...
    def changeDinerToEating(self, chairIndex):
        ...
        foodChat = [
         ...
         "It's about time.",
         'Hmmph.']
        diner.nametag3d.unstash()
        diner.setChatAbsolute(random.choice(foodChat), CFSpeech | CFTimeout)
        taskMgr.doMethodLater(self.CHAT_DELAY, self.stashTag, self.nametagName, extraArgs=[diner])
    ...
    def changeDinerToAngry(self, chairIndex):
        ...
        angryChat = [
         ...
         'This is an outrage!',
         "I'm never coming back this week!",
         "I'm calling your boss!"]
        diner.nametag3d.unstash()
        diner.setChatAbsolute(random.choice(angryChat), CFSpeech | CFTimeout)
        taskMgr.doMethodLater(self.CHAT_DELAY, self.stashTag, self.nametagName, extraArgs=[diner])

    def stashTag(self, diner):
        diner.nametag3d.stash()
        return Task.done
    ...
    def enterFlat(self, avId):
        ...
        if self.avId and self.avId == localAvatar.doId:
            self.flattenLocalToon(ToontownGlobals.BossCogDamageLevels[ToontownGlobals.BossCogMoveAttack])
    ...
    def flattenLocalToon(self, damage):
        if not localAvatar.isStunned:
            base.cr.playGame.getPlace().fsm.request('flatten')
            localAvatar.stunToon()
            localAvatar.setZ(localAvatar.getZ(render) + 0.025)
            localAvatar.d_squish(damage)
            localAvatar.playDialogueForString('!')
