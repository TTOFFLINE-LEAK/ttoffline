...
class CountryClubInterior(BattlePlace.BattlePlace):
    ...
    def load(self):
        ...
        if musicName.endswith('v2.ogg'):
            self.isSecond = True
            self.loopMusic = base.loader.loadMusic('phase_12/audio/bgm/Bossbot_Factory_v2_loop.ogg')
            self.musicSeq = Sequence(Func(base.playMusic, self.music, looping=0, volume=0.8), Wait(30.262), Func(base.playMusic, self.loopMusic, looping=1, volume=0.8))
