...
from toontown.shtiker import ShtikerBook, InventoryPage, MapPage, OptionsPage, ShardPage, QuestPage, TrackPage, WordPage, ServerPage, KartPage, GardenPage, GolfPage
from toontown.shtiker import SuitPage, DisguisePage, PhotoAlbumPage, FishPage, NPCFriendPage, EventsPage, TIPPage
...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        try:
            ...
            if config.GetBool('want-retro-mode', False):
                self.soundRun = base.loader.loadSfx('phase_3.5/audio/sfx/AV_footstep_runloop_tto.ogg')
            else:
            ...
            self.useWaterSound = False
            self.useDiscSound = False
            ...
            if config.GetBool('want-retro-mode', False):
                self.soundSystemMessage = base.loader.loadSfx('phase_3.5/audio/sfx/GUI_whisper_3.ogg')
            else:
                self.soundSystemMessage = base.loader.loadSfx('phase_4/audio/sfx/clock03.ogg')
            ...
            self.questMap = None
            self.prevToonIdx = 0
            self.chairmanReveal = False
            self.christmasDay = False
            self.doorIgnore = False
    ...
    def setDNAString(self, dnaString):
        if base.cr.avIsInEpisode():
            dnaString = EpisodeConfig.episodes[base.cr.currentEpisode]['toon'].get('dna', ('dss',
                                                                                           'ms',
                                                                                           'm',
                                                                                           'm',
                                                                                           0,
            ...
            dnaString = dna.makeNetString()
        DistributedToon.DistributedToon.setDNAString(self, dnaString)
    ...
    def setName(self, name):
        if base.cr.avIsInEpisode():
            name = EpisodeConfig.episodes[base.cr.currentEpisode]['toon'].get('name', 'Episode Toon')
    ...
    def initInterface(self):
        ...
        if not __debug__:
            ...
            self.serverPage = ServerPage.ServerPage()
            self.serverPage.load()
            self.book.addPage(self.serverPage, pageName=TTLocalizer.ServerPageTitle)
        ...
        if not base.cr.isPaid():
            ...
            self.purchaseButton = DirectButton(parent=base.a2dBottomRight, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=0.9, text=TTLocalizer.OptionsPagePurchase, text_scale=0.05, text_pos=(0,
                                                                                                                                                                                                                                                                                       -0.01), textMayChange=0, pos=(-0.448,
                                                                                                                                                                                                                                                                                                                     0,
                                                                                                                                                                                                                                                                                                                     0.06), sortOrder=100, command=self.__handlePurchase)
        ...
        if base.sosCheck:
            ...
            whisper = WhisperPopup('NOTICE: You need to update your SOS.json.  Please contact the TTOff Team for more information.', OTPGlobals.getInterfaceFont(), WhisperPopup.WTEmote)
    ...
    def generateToonColor(self):
        DistributedToon.DistributedToon.generateToonColor(self)
        if self.laffMeter:
            self.laffMeter.adjustToonColor(self.style)
    ...
    def updatePieButton(self):
        ...
        if self.__pieButton == None:
            inv = self.inventory
            if self.pieType == ToontownGlobals.PieEMP:
                gui = loader.loadModel('phase_3.5/models/gui/stickerbook_gui')
                pieGui = gui.find('**/summons')
                empTex = loader.loadTexture('phase_3.5/maps/emp.png')
                pieTs = pieGui.findTextureStage('*')
                pieGui.setTexture(pieTs, empTex, 1)
                pieGui.setTransparency(1)
                pieScale = 0.1
            else:
                if self.pieType == ToontownGlobals.PieSnowball:
                    gui = loader.loadModel('phase_3.5/models/gui/stickerbook_gui')
                    pieGui = gui.find('**/summons')
                    snowTex = loader.loadTexture('phase_3.5/maps/snowballGUI.png')
                    pieTs = pieGui.findTextureStage('*')
                    pieGui.setTexture(pieTs, snowTex, 1)
                    pieGui.setTransparency(1)
                    pieScale = 0.1
                else:
                    if self.pieType == ToontownGlobals.PieEvidence:
                        gui = loader.loadModel('phase_3.5/models/gui/stickerbook_gui')
                        pieGui = gui.find('**/summons')
                        pieScale = 0.1
                    else:
    ...
    def displayWhisper(self, fromId, chatString, whisperType):
        sender = None
        sfx = self.soundWhisper
        whisperFont = ToontownGlobals.getInterfaceFont()
        ...
        if whisperType == WhisperPopup.WTNormal or whisperType == WhisperPopup.WTQuickTalker:
            ...
            if whisperType == WhisperPopup.WTSystem:
                sfx = self.soundSystemMessage
            else:
                if whisperType == WhisperPopup.WTSwagForeman:
                    chatString = TTLocalizer.SuitSwagForeman + ': ' + chatString
                    whisperFont = ToontownGlobals.getSuitFont()
        whisper = WhisperPopup(chatString, whisperFont, whisperType)
        if sender != None:
    ...
    def showToonfestGui(self):
        if base.cr.playGame.getPlace().getZoneId() == ToontownGlobals.ToonFest:
            self.loadToonfestGui()
            self.__toonfestButton['text'] = ['', 'TOONFEST!', 'TOONFEST!']
            self.__toonfestButton.show()

    def hideToonfestGui(self):
        if self.__toonfestButton:
            self.__toonfestButton.hide()
            self.__toonfestFlash.pause()

    def loadToonfestGui(self):
        if self.__toonfestButton:
            return
        icon = loader.loadModel('phase_3.5/models/gui/toonfest_gui')
        self.__toonfestButton = DirectButton(relief=None, image=icon, text='', text_fg=(1,
                                                                                        1,
                                                                                        1,
                                                                                        1), text_shadow=(0,
                                                                                                         0,
                                                                                                         0,
                                                                                                         1), text_scale=0.2, text_pos=(0,
                                                                                                                                       0.05), text_font=ToontownGlobals.getInterfaceFont(), pos=(-0.4,
                                                                                                                                                                                                 1.0,
                                                                                                                                                                                                 -0.12), scale=0.24, command=self.__handleToonfestButton)
    ...
    def showHillsGui(self):
        if base.cr.playGame.getPlace().getZoneId() != ToontownGlobals.ToonFest and base.cr.holidayValue == 1 and base.cr.isHalloween:
            self.loadHillsGui()
            self.__hillsButton['text'] = ['', "Take me to\nBill's Hills!", "Take me to\nBill's Hills!"]
            self.__hillsButton.show()

    def hideHillsGui(self):
        if self.__hillsButton:
            self.__hillsButton.hide()
            self.__hillsFlash.pause()

    def loadHillsGui(self):
        if self.__hillsButton:
            return
        icon = loader.loadModel('phase_3.5/models/gui/hills_gui')
        self.__hillsButton = DirectButton(relief=None, image=icon, text='', text_fg=(1,
                                                                                     1,
                                                                                     1,
                                                                                     1), text_shadow=(0,
                                                                                                      0,
                                                                                                      ...
                                                                                                      1), text_scale=0.2, text_pos=(0,
                                                                                                                                    0.05), text_font=ToontownGlobals.getInterfaceFont(), pos=(-0.4,
                                                                                                                                                                                              1.0,
                                                                                                                                                                                              -0.12), scale=0.24, command=self.__handleHillsButton)
        self.__hillsButton.reparentTo(base.a2dTopRight, DGG.BACKGROUND_SORT_INDEX - 1)
        return

    def __handleHillsButton(self):
        base.localAvatar.magicTeleportInitiate(7000, 7200)
    ...
    def clarabelleNewsPageCollision(self, show=True):
        ...
        if show:
            ...
            newPos = (claraXPos - 0.1, 1.0, -0.56)
        else:
            if WantNewsPage:
                newPos = (
                 claraXPos - 0.1, 1.0, -0.56)
            else:
                newPos = (
                 claraXPos, 1.0, -0.63)
    ...
    def showClarabelleGui(self, mailboxItems):
        ...
        if base.cr.inGameNewsMgr and localAvatar.getLastTimeReadNews() < base.cr.inGameNewsMgr.getLatestIssue() and config.GetBool('want-news-tab', 1):
            self.clarabelleNewsPageCollision(True)
        else:
            self.clarabelleNewsPageCollision(False)
        ...
        if self.friendsListButtonActive and self.friendsListButtonObscured <= 0:
            ...
            if self.clarabelleButtonObscured <= 0 and self.isTeleportAllowed():
                if self.catalogNotify == ToontownGlobals.NewItems or self.mailboxNotify == ToontownGlobals.NewItems or self.simpleMailNotify == ToontownGlobals.NewItems or self.inviteMailNotify == ToontownGlobals.NewItems or self.awardNotify == ToontownGlobals.NewItems:
                    showClarabelle = (not launcher or launcher.getPhaseComplete(5.5)) and not base.config.GetBool('want-doomsday', False)
    ...
    def newCatalogNotify(self):
        ...
        if self.mailboxNotify == ToontownGlobals.NoItems:
            ...
            if self.mailboxNotify == ToontownGlobals.NewItems:
                if self.catalogNotify == ToontownGlobals.NewItems:
                    message = (
                ...
                if self.mailboxNotify == ToontownGlobals.OldItems:
                    if self.catalogNotify == ToontownGlobals.NewItems:
                        message = (
        ...
        if self.awardNotify == ToontownGlobals.NoItems:
            ...
            if self.awardNotify == ToontownGlobals.NewItems:
                ...
                if self.awardNotify == ToontownGlobals.OldItems:
                    oldStr = ''
                    if message:
                        oldStr = message[0] + ' '
    ...
    def setSpeed(self, forwardSpeed, rotateSpeed):
        action = DistributedToon.DistributedToon.setSpeed(self, forwardSpeed, rotateSpeed)
        if self.standWalkRunReverse != None:
            replay = 0
            showWake, wakeWaterHeight = ZoneUtil.getWakeInfo()
            if self.isDisguised and not self.useDiscSound and config.GetBool('cog-footsteps', True):
                if self.soundRun.status() == self.soundRun.PLAYING:
                    replay = 1
                    self.soundRun.stop()
                base.loader.unloadSfx(self.soundRun)
                if self.soundWalk.status() == self.soundWalk.PLAYING:
                    replay = 2
                    self.soundWalk.stop()
                base.loader.unloadSfx(self.soundWalk)
                ext = 'sm'
                if self.suit.getHeight() >= 6.4:
                    ext = 'lg'
                coolString = 'phase_5/audio/sfx/ENC_cog' + self.suit.style.body.lower() + 'fs' + ext + '.ogg'
                self.soundRun = base.loader.loadSfx(coolString)
                self.soundWalk = base.loader.loadSfx(coolString)
                self.useWaterSound = False
                self.useDiscSound = True
            else:
                if showWake and self.getZ(render) < wakeWaterHeight and not self.useWaterSound and not self.useDiscSound:
                    if self.soundRun.status() == self.soundRun.PLAYING:
                        replay = 1
                        self.soundRun.stop()
                    base.loader.unloadSfx(self.soundRun)
                    ...
                    base.loader.unloadSfx(self.soundWalk)
                    if config.GetBool('want-retro-mode', False):
                        self.soundRun = base.loader.loadSfx('phase_4/audio/sfx/AV_footstep_runloop_water_tto.ogg')
                    else:
                        self.soundRun = base.loader.loadSfx('phase_4/audio/sfx/AV_footstep_runloop_water.ogg')
                    self.soundRun.setVolume(2)
                    self.soundWalk = base.loader.loadSfx('phase_4/audio/sfx/AV_footstep_walkloop_water.ogg')
                    self.useWaterSound = True
                else:
                    if not (showWake and self.getZ(render) < wakeWaterHeight) and self.useWaterSound or self.useDiscSound and not self.isDisguised:
                        if self.soundRun.status() == self.soundRun.PLAYING:
                            replay = 1
                            self.soundRun.stop()
                        base.loader.unloadSfx(self.soundRun)
    ...
    def swimTimeoutAction(self):
        self.ignore('wakeup')
        if not base.cr.newsManager.isHolidayRunning(ToontownGlobals.ORANGES):
            self.takeOffSuit()
    ...
    def getAccountDays(self):
        ...
        if defaultDays >= 0:
            ...
            if hasattr(base.cr, 'accountDays'):
    ...
    def setIsTeleporting(self, isTeleporting):
        self.isTeleporting = isTeleporting

    def getIsTeleporting(self):
        return self.isTeleporting

    def brickLay(self, type='none'):
        if self.bricks or type == 'none':
            self.bricks.destroy()
            self.bricks = None
        if type == 'lab':
            img = 'phase_14/maps/episode_bars.png'
        else:
            if type == 'toontowncentral':
                img = 'phase_14/maps/episode_ttc.png'
            else:
                if type == 'daisygardens':
                    img = 'phase_14/maps/episode_wood.png'
                else:
                    if type == 'cog':
                        img = 'phase_14/maps/episode_cog.png'
        if type != 'none':
            self.bricks = OnscreenImage(image=img, scale=(2.1, 1, 1))
            self.bricks.setTransparency(TransparencyAttrib.MAlpha)
        return

    def flashLogo(self, length):
        wait = length - 4
        logo = OnscreenImage(image='phase_14/maps/DH_Logo.png', scale=(0.7, 0.94, 0.5))
        logo.setTransparency(TransparencyAttrib.MAlpha)
        logo.setColorScale(1, 1, 1, 0)
        seq = Sequence(logo.colorScaleInterval(2, (1, 1, 1, 1)), Wait(wait), logo.colorScaleInterval(2, (1,
                                                                                                         1,
                                                                                                         1,
                                                                                                         0)), Func(logo.destroy))
        seq.start()