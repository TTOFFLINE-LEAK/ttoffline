...
PURCHASE_MOVIE_ITEM_BOUGHT = 12
PURCHASE_MOVIE_ITEM_LIMIT = 13
PURCHASE_MOVIE_ITEM_FULL = 14
PURCHASE_MOVIE_ITEM_INVALID = 15
PURCHASE_MOVIE_MAIL_FULL = 16
PURCHASE_MOVIE_DELIVERY_FULL = 17
PURCHASE_MOVIE_BAD_SITUATION = 18
SELL_MOVIE_CLEAR = 0
SELL_MOVIE_START = 1
...
def determinePosIndex(name):
    if name == TTLocalizer.NPCToonNames.get(5012):
        return 1
    if name == TTLocalizer.NPCToonNames.get(1012):
        return 1
    if name == TTLocalizer.NPCToonNames.get(1013):
        return 2
    return -1


def createNPC(air, npcId, desc, zoneId, posIndex=0, questCallback=None):
    import DistributedNPCToonAI, DistributedNPCClerkAI, DistributedNPCTailorAI, DistributedNPCBlockerAI, DistributedNPCFishermanAI, DistributedNPCPetclerkAI, DistributedNPCKartClerkAI, DistributedNPCPartyPersonAI, DistributedNPCSpecialQuestGiverAI, DistributedNPCFlippyInToonHallAI, DistributedNPCScientistAI, DistributedNPCSnowballGiverAI, DistributedNPCRoamingAI, DistributedNPCPrizeClerkAI, DistributedNPCMagicAI
    if len(desc) == 6:
                                            ...
                                            if type == NPC_SPECIALQUESTGIVER:
                                                ...
                                                if type == NPC_FLIPPYTOONHALL:
                                                    ...
                                                    if type == NPC_SCIENTIST:
                                                        ...
                                                        if type == NPC_SNOWBALLGIVER:
                                                            npc = DistributedNPCSnowballGiverAI.DistributedNPCSnowballGiverAI(air, npcId)
                                                        else:
                                                            if type == NPC_ROAMING:
                                                                npc = DistributedNPCRoamingAI.DistributedNPCRoamingAI(air, npcId)
                                                            else:
                                                                if type == NPC_MAGIC:
                                                                    npc = DistributedNPCMagicAI.DistributedNPCMagicAI(air, npcId)
                                                                else:
    ...
    npc.setHp(15)
    npc.setMaxHp(15)
    newPosIndex = determinePosIndex(npc.getName())
    if newPosIndex == -1:
        newPosIndex = posIndex
    npc.setPositionIndex(newPosIndex)
...
def createNpcsInZone(air, zoneId):
    if zoneId == ToontownGlobals.ToontownCentral and config.GetBool('want-doomsday', False):
        return []
    npcs = []
    ...
    for i in range(len(npcIdList)):
        ...
        type = npcDesc[(-1)]
        if type == NPC_SNOWBALLGIVER:
            if not (simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WINTER_DECORATIONS) or simbase.air.holidayManager.isHolidayRunning(ToontownGlobals.WINTER_CAROLING)):
                continue
...
NPCToonDict = {1: (
   ...
   20001: (
         ...
         1,
         NPC_BLOCKER), 
   2001: 'Flippy will be defined later on in the file.', 
   2002: (
        2514,
        lnames[2002],
        (
   ...
   6010: (
        ...
        1,
        NPC_FISHERMAN), 
   14001: 'Resistance Ranger Rocky will be defined later on in the file.', 
   14002: (
         -1,
         lnames[14002],
         (
