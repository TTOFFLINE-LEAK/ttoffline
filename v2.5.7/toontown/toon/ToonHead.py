...
if not config.GetBool('want-new-anims', 1):
    HeadDict = {'dls': '/models/char/dogMM_Shorts-head-', 'dss': '/models/char/dogMM_Skirt-head-', 'dsl': '/models/char/dogSS_Shorts-head-', 
       ...
       'i': '/models/char/gyro-heads-', 
       'gsl': '/models/char/tt_a_chr_riggy_head_', 
       'gss': '/models/char/tt_a_chr_riggy_head_', 
       'gls': '/models/char/tt_a_chr_riggy_head_', 
       'gll': '/models/char/tt_a_chr_riggy_head_', 
       'g': '/models/char/tt_a_chr_riggy_head_'}
else:
...
EyelashDict = {'d': '/models/char/dog-lashes', 'c': '/models/char/cat-lashes', 'h': '/models/char/horse-lashes', 
   ...
   'i': '/models/char/gyro-lashes', 
   'g': '/models/char/rabbit-lashes'}
...
class ToonHead(Actor.Actor):
    ...
    EyesRiggy = loader.loadTexture('phase_3/maps/eyesriggy2.jpg', 'phase_3/maps/eyesriggy2_a.rgb')
    EyesRiggy.setMinfilter(Texture.FTLinear)
    EyesRiggy.setMagfilter(Texture.FTLinear)
    ...
    def generateToonHead(self, copy, style, lods, forGui=0):
        ...
        fix = None
        speciesType = headStyle[0]
        headSize = headStyle[1] + headStyle[2]
        if headStyle in ToonDNA.toonHeadTypes:
            if speciesType == 'd':
                filePrefix = HeadDict[headStyle]
            else:
                filePrefix = HeadDict[speciesType]
            if headSize == 'ls':
        ...
        for lod in lods:
            ...
            if not multiLod:
                ...
                lodFix = None
            if headStyle[0] == 'g':
                self.loadModel('phase_3' + filePrefix.strip('_'), 'head', lodLoad, copy)
            else:
                self.loadModel('phase_3' + filePrefix + lod, 'head', lodLoad, copy)
            if not forGui:
                pLoaded = self.loadPumpkin(headStyle[1], lodFix, copy)
                self.loadSnowMan(headStyle[1], lodFix, copy)
            if not copy:
                self.showAllParts('head')
            if fix != None and speciesType != 'd':
                fix(style, lodFix, copy)
            if not forGui:
                if pLoaded:
                    self.__fixPumpkin(style, lodFix, copy)
                else:
                    self.__lods = lods
                    self.__style = style
    ...
    def generateToonColor(self, style):
        ...
        for part in parts:
            if style.headColor == 39:
                magicParallel.append(Sequence(LerpColorInterval(part, duration=1, color=VBase4(0.996094, 0.898438, 0.320312, 1.0)), LerpColorInterval(part, duration=1, color=VBase4(0.304688, 0.96875, 0.402344, 1.0)), LerpColorInterval(part, duration=1, color=VBase4(0.191406, 0.5625, 0.773438, 1.0)), LerpColorInterval(part, duration=0.5, color=VBase4(0.546875, 0.28125, 0.75, 1.0)), LerpColorInterval(part, duration=0.5, color=VBase4(0.933594, 0.265625, 0.28125, 1.0))))

        animalType = style.getAnimal()
        if animalType == 'cat' or animalType == 'rabbit' or animalType == 'bear' or animalType == 'mouse' or animalType == 'pig' or animalType == 'riggy':
        ...
        if animalType == 'riggy':
            parts = self.findAllMatches('**/head-short')
            parts.setColor(style.getHeadColor())
            for part in parts:
                if style.headColor == 39:
                    magicParallel.append(Sequence(LerpColorInterval(part, duration=1, color=VBase4(0.996094, 0.898438, 0.320312, 1.0)), LerpColorInterval(part, duration=1, color=VBase4(0.304688, 0.96875, 0.402344, 1.0)), LerpColorInterval(part, duration=1, color=VBase4(0.191406, 0.5625, 0.773438, 1.0)), LerpColorInterval(part, duration=0.5, color=VBase4(0.546875, 0.28125, 0.75, 1.0)), LerpColorInterval(part, duration=0.5, color=VBase4(0.933594, 0.265625, 0.28125, 1.0))))

        if not self.fullToon and style.headColor == 39:
        ...
        if not self.__eyes.isEmpty():
            ...
            if lp.isEmpty() or rp.isEmpty():
                ...
                if config.GetBool('want-retro-mode', False):
                    if animalType != 'horse':
                        if animalType == 'pig' or animalType == 'bear':
                            eyeTex = loader.loadTexture('phase_3/maps/avatar_palette_2tmla_1_retro.jpg', 'phase_3/maps/avatar_palette_2tmla_1_retro_a.rgb')
                        else:
                            eyeTex = loader.loadTexture('phase_3/maps/pupils_retro.jpg', 'phase_3/maps/pupils_retro_a.rgb')
                        lp.setTexture(eyeTex, 1)
                        rp.setTexture(eyeTex, 1)
                    else:
                        lp.setTexture(loader.loadTexture('phase_3/maps/avatar_palette_2tmla_1_retro.jpg', 'phase_3/maps/avatar_palette_2tmla_1_retro_a.rgb'), 1)
                        rp.setTexture(loader.loadTexture('phase_3/maps/pupils_retro.jpg', 'phase_3/maps/pupils_retro_a.rgb'), 1)
                ...
                if config.GetBool('want-retro-mode', False):
                    if style.head[0] + style.head[1] == 'ds':
                        ...
                        if style.head[2] == 'l':
                            x = 0.019
                            z = 0.025
                        lpt.setPos(lpt.getX() + x, lpt.getY() + 0.05, lpt.getZ() + z)
                        lpt.setScale(0.9)
                        rpt.setPos(rpt.getX() - x, rpt.getY() + 0.05, rpt.getZ() + z)
                        rpt.setScale(0.9)
                lp.reparentTo(lpt)
                rp.reparentTo(rpt)
    ...
    def cashEyes(self, abc):
        if abc:
            if self.__lpupil:
                self.__lpupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
                self.__rpupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
            if self.__lod500lPupil:
                self.__lod500lPupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
                self.__lod500rPupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
            if self.__lod250lPupil:
                self.__lod250lPupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
                self.__lod250rPupil.setTexture(loader.loadTexture('phase_3/maps/pupils-c.png'), 1)
            print 'm'
        else:
            self.__lpupil.setTexture(loader.loadTexture('phase_3/maps/pupils.jpg', 'phase_3/maps/pupils_a.rgb'), 1)
            self.__rpupil.setTexture(loader.loadTexture('phase_3/maps/pupils.jpg', 'phase_3/maps/pupils_a.rgb'), 1)
    ...
    def __fixHeadLongShort(self, style, lodName=None, copy=1):
        ...
        if animalType == 'riggy':
            return
        if animalType != 'duck' and animalType != 'horse' and animalType != 'scrooge' and animalType != 'gyro':
    ...
    def riggyEyes(self):
        self.__eyesOpen = ToonHead.EyesRiggy
        self.__eyesClosed = ToonHead.EyesClosed
