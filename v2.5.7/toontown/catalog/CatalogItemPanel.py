...
class CatalogItemPanel(DirectFrame):
    ...
    def __init__(self, parent=aspect2d, parentCatalogScreen=None, useTokens=False, **kw):
    ...
    def load(self):
        ...
        if self['item'].needsCustomize() and (typeCode == CatalogItemTypes.WALLPAPER_ITEM or typeCode == CatalogItemTypes.FLOORING_ITEM or typeCode == CatalogItemTypes.MOULDING_ITEM or typeCode == CatalogItemTypes.FURNITURE_ITEM or typeCode == CatalogItemTypes.WAINSCOTING_ITEM or typeCode == CatalogItemTypes.TOON_STATUE_ITEM):
            if typeCode == CatalogItemTypes.WALLPAPER_ITEM:
                ...
                if typeCode == CatalogItemTypes.FLOORING_ITEM:
                    self.items = getAllFloorings(self['item'].patternIndex)
                else:
                    if typeCode == CatalogItemTypes.MOULDING_ITEM:
                        self.items = getAllMouldings(self['item'].patternIndex)
                    else:
                        if typeCode == CatalogItemTypes.FURNITURE_ITEM:
                            self.items = getAllFurnitures(self['item'].furnitureType)
                        else:
                            if typeCode == CatalogItemTypes.TOON_STATUE_ITEM:
                                self.items = self['item'].getAllToonStatues()
                            else:
                                if typeCode == CatalogItemTypes.WAINSCOTING_ITEM:
        ...
        if self['item'].getTypeCode() == CatalogItemTypes.CHAT_ITEM:
            ...
            if self['item'].getTypeCode() == CatalogItemTypes.ACCESSORY_ITEM:
        ...
        if self.useTokens:
            priceStr = str(numericBeanPrice) + ' ' + TTLocalizer.CatalogToonfestCurrency
        else:
    ...
    def getTeaserPanel(self):
        ...
        if typeName == TTLocalizer.EmoteTypeName or typeName == TTLocalizer.ChatTypeName:
            ...
            if typeName == TTLocalizer.GardenTypeName or typeName == TTLocalizer.GardenStarterTypeName:
                page = 'gardening'
            else:
                page = 'clothing'
    ...
    def updateBuyButton(self):
        ...
        if isNameTag and not localAvatar.getGameAccess() == OTPGlobals.AccessFull:
            ...
            elif self.useTokens and self['item'].getPrice(self['type']) > base.localAvatar.getTokens():
                self.buyButton['state'] = DGG.DISABLED
            ...
            if isNameTag and self['item'].nametagStyle == localAvatar.getNametagStyle():
                ...
                if self['item'].reachedPurchaseLimit(base.localAvatar):
                    ...
                    if hasattr(self['item'], 'noGarden') and self['item'].noGarden(base.localAvatar):
                        auxText = TTLocalizer.NoGarden
                        self.buyButton['state'] = DGG.DISABLED
                    else:
                        if hasattr(self['item'], 'isSkillTooLow') and self['item'].isSkillTooLow(base.localAvatar):
                            auxText = TTLocalizer.SkillTooLow
                            self.buyButton['state'] = DGG.DISABLED
                        else:
                            if hasattr(self['item'], 'getDaysToGo') and self['item'].getDaysToGo(base.localAvatar):
                                auxText = TTLocalizer.DaysToGo % self['item'].getDaysToGo(base.localAvatar)
                                self.buyButton['state'] = DGG.DISABLED
                            else:
                                if self['item'].getEmblemPrices() and not base.localAvatar.isEnoughMoneyAndEmblemsToBuy(self['item'].getPrice(self['type']), self['item'].getEmblemPrices()):
                                    self.buyButton['state'] = DGG.DISABLED
                                else:
                                    if self.useTokens and self['item'].getPrice(self['type']) <= base.localAvatar.getTokens():
    ...
    def __handlePurchaseRequest(self):
        if self['item'].replacesExisting() and self['item'].hasExisting():
            ...
            if self['item'].isRental():
                ...
                if self.useTokens:
                    message = TTLocalizer.CatalogVerifyPurchaseTokens % {'item': self['item'].getName(), 'price': self['item'].getPrice(self['type'])}
                else:
                    emblemPrices = self['item'].getEmblemPrices()
                    if emblemPrices:
