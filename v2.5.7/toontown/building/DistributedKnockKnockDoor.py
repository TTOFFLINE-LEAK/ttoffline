...
class DistributedKnockKnockDoor(DistributedAnimatedProp.DistributedAnimatedProp):
    ...
    def setupTrigger(self):
        buildings = render.findAllMatches('**/tb%s*' % str(self.propId))
        for building in buildings:
            trigger = building.find('**/door_*/+CollisionNode')
            if trigger:
                if trigger.getName() != 'KnockKnockDoorSphere_' + str(self.propId):
                    trigger.setName('KnockKnockDoorSphere_' + str(self.propId))
