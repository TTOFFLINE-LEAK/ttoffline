...
class DistributedBoardingPartyAI(DistributedObjectAI.DistributedObjectAI, BoardingPartyBase.BoardingPartyBase):
    ...
    def requestInvite(self, inviteeId):
        ...
        if not isinstance(invitee, DistributedToonAI.DistributedToonAI):
            self.notify.warning(('{0} tried to invite non-Toon object {1} to their boarding group').format(inviterId, inviteeId))
            reason = BoardingPartyBase.BOARDCODE_BAD_DO
