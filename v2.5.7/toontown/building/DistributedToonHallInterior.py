...
class DistributedToonHallInterior(DistributedToonInterior):
    ...
    def __init__(self, cr):
        ...
        self.sillyFSM = ClassicFSM.ClassicFSM('SillyOMeter', [
         State.State('Setup', self.enterSetup, self.exitSetup, ['Phase0',
          ...
          'Flat',
          'Off']),
         State.State('Phase0', self.enterPhase0, self.exitPhase0, ['Phase1',
          'Phase2',
          'Phase3',
          'Phase4',
          'Phase5',
    ...
    def togglePanel(self, avId=None):
        if not avId:
            return
        ...
        if toon == base.localAvatar:
            podium = self.interior.find('**/anvil_podium')
            display = self.interior.find('**/Anvil')
            podiumShake = Sequence(Wait(0.5))
            shake = 0.05
            for x in range(0, 10):
                podiumShake.append(podium.posInterval(0.1, (podium.getX(), shake, podium.getZ())))
                podiumShake.append(podium.posInterval(0.1, (podium.getX(), -shake, podium.getZ())))
                shake = shake + 0.1

            podiumShake.append(podium.posInterval(0.5, (podium.getX(), podium.getY(), podium.getZ() - 10)))
            podiumShake.append(Wait(0.5))
            podiumShake.append(display.posInterval(0.5, (display.getX(), display.getY(), display.getZ() - 10)))
            seq = Sequence(Func(podiumShake.start), Wait(3), self.interior.find('**/anvil_wall').posInterval(3, (0,
                                                                                                                 0,
                                                                                                                 13)))
            seq.start()
    ...
    def enterSetup(self):
        ...
        self.sillyMeter.setBlend(frameBlend=config.GetBool('interpolate-animations', True))
        ...
        self.accept('SillyMeterDuration', self.setDuration)
        self.startIfNeeded()
    ...
    def enterFlat(self):
        ...
        sillyMeterSign = self.interior.find('**/sillyOMeterSign')
        if sillyMeterSign:
