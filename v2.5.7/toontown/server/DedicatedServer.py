...
UBERDOG_DONE_MSG = 'UberDOG has successfully initialized.'
AI_DONE_MSG = 'District is now ready. Have fun in Toontown!'
...
class DedicatedServer:
    ...
    def start(self):
        ...
        if config.GetBool('auto-start-local-server', True) and not self.isLocal and not config.GetBool('want-mini-server', False):
        ...
        if sys.platform == 'win32':
            self.astronProcess = subprocess.Popen('astron\\astrond.exe --loglevel info astron\\config\\astrond.yml', stdin=self.astronLog, stdout=self.astronLog, stderr=self.astronLog)
        else:
            if sys.platform == 'linux2':
                self.astronProcess = subprocess.Popen([
                 './astron/astrond-linux', '--loglevel', 'info', 'astron/config/astrond.yml'], stdin=self.astronLog, stdout=self.astronLog, stderr=self.astronLog)
            else:
                self.astronProcess = subprocess.Popen([
                 './astron/astrond', '--loglevel', 'info', 'astron/config/astrond.yml'], stdin=self.astronLog, stdout=self.astronLog, stderr=self.astronLog)
    ...
    def serverStarted(self, task):
        ...
        self.notify.info('Server is now ready. Have fun in Toontown!')
        if self.isLocal:
