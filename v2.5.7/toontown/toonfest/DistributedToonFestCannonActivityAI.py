...
class DistributedToonFestCannonActivityAI(DistributedPartyActivityAI):
    ...
    def setLanded(self, toonId):
        ...
        if self.isTF:
            if reward > 0:
                self.sendUpdateToAvatarId(avId, 'showTokenReward', [reward, av.getTokens(),
                 TTLocalizer.ToonFestCannonResults % (
                  reward, self.cloudsHit[avId])])
                av.addTokens(reward)
        else:
            if reward > 0:
                self.sendUpdateToAvatarId(avId, 'showJellybeanReward', [reward, av.getMoney(),
    ...
    def setToonTrajectoryAi(self, launchTime, x, y, z, h, p, r, vx, vy, vz):
        self.sendUpdate('setToonTrajectory', [
         self.air.getAvatarIdFromSender(), launchTime, x, y, z, h, p, r, vx, vy, vz])
