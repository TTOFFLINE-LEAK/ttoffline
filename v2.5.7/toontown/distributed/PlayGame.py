...
class PlayGame(StateData.StateData):
    ...
    def loadDnaStoreTutorial(self):
        ...
        files = ('phase_3.5/dna/storage_tutorial.jazz', 'phase_3.5/dna/storage_interior.jazz')
    ...
    def loadDnaStore(self):
        if not hasattr(self, 'dnaStore'):
            self.dnaStore = DNAStorage()
            files = ('phase_4/dna/storage.jazz', 'phase_3.5/dna/storage_interior.jazz')
            dnaBulk = DNABulkLoader(self.dnaStore, files)
            dnaBulk.loadDNAFiles()
    ...
    def handleHoodDone(self):
        ...
        if doneStatus['where'] == 'party':
            ...
            return
        how = doneStatus['how']
        if how in ('tunnelIn', 'teleportIn', 'doorIn', 'elevatorIn', 'trainIn', 'walk'):
