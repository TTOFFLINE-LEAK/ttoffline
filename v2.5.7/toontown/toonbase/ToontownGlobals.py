...
from otp.otpbase.PythonUtil import Enum, invertDict
...
CogHQCameraNear = 1.0
CreditCameraFar = 3000.0
CreditCameraNear = 1.0
...
def getComicFont():
    global ComicFont
    if ComicFont == None:
        ComicFont = loader.loadFont(TTLocalizer.ComicFont, lineHeight=1.0)
    return ComicFont
...
def getNametagFont(index):
    if index not in NametagFonts or NametagFonts[index] is None:
        if index not in NametagFontPaths or NametagFontPaths[index] is None:
            ...
            NametagFonts[index] = getSuitFont()
        else:
...
HoodHierarchy = {ToontownCentral: (SillyStreet, LoopyLane, PunchlinePlace), DonaldsDock: (
   ...
   ToonFest: (
            ToonFestCogCrusherCourse, BillsHills), 
   GoofySpeedway: ()}
...
SellbotWestWing = 11300
SellbotFactoryInt = 11500
SwagFactoryInt = 11569
CashbotHQ = 12000
CashbotLobby = 12100
CashbotShortChangeOffice = 12200
CashbotHighriseHallway = 12210
CashbotBathroom = 12220
CashbotBar = 12230
CashbotBuffOffice = 12240
CashbotMintIntA = 12500
CashbotMintIntB = 12600
CashbotMintIntC = 12700
ShortWorkZones = [CashbotShortChangeOffice, CashbotHighriseHallway, CashbotBathroom, CashbotBar, CashbotBuffOffice]
LawbotHQ = 13000
LawbotLobby = 13100
...
GolfZone = 17000
PartyHood = 18000
HoodsAlwaysVisited = [14000, 17000, 18000]
...
HoodsForTeleportAll = (DonaldsDock,
 ...
 LawbotHQ,
 GolfZone)
BingoCardNames = {'normal': 0, 'corners': 1, 
   'diagonal': 2, 
   'threeway': 3, 
   'blockout': 4}
NoPreviousGameId = 0
...
MinigameIDs = (RaceGameId,
 ...
 PhotoGameId,
 TravelGameId)
MinigamePlayerMatrix = {1: (
     CannonGameId, MazeGameId, TugOfWarGameId, RingGameId, VineGameId, CogThiefGameId, TwoDGameId, DivingGameId, PairingGameId, CatchGameId, TargetGameId, PhotoGameId), 
   2: (
     CannonGameId, MazeGameId, TugOfWarGameId, PatternGameId, TagGameId, RingGameId, VineGameId, IceGameId, CogThiefGameId, TwoDGameId, DivingGameId, PairingGameId, CatchGameId, TargetGameId, PhotoGameId), 
   3: (
     CannonGameId, MazeGameId, TugOfWarGameId, PatternGameId, RaceGameId, TagGameId, VineGameId, RingGameId, IceGameId, CogThiefGameId, TwoDGameId, DivingGameId, PairingGameId, CatchGameId, TargetGameId, PhotoGameId), 
   4: (
     CannonGameId, MazeGameId, TugOfWarGameId, PatternGameId, RaceGameId, TagGameId, VineGameId, RingGameId, IceGameId, CogThiefGameId, TwoDGameId, DivingGameId, PairingGameId, CatchGameId, TargetGameId, PhotoGameId)}
...
dnaMap = {Tutorial: 'toontown_central', ToontownCentral: 'toontown_central', 
   ...
   OldDaisyGardens: 'old_daisys_garden', 
   ScroogeBank: 'scrooge_bank', 
   ToonFest: 'toonfest', 
...
hoodNameMap = {DonaldsDock: TTLocalizer.DonaldsDock, ToontownCentral: TTLocalizer.ToontownCentral, 
   ...
   OldDaisyGardens: TTLocalizer.OldDaisyGardens, 
   ScroogeBank: TTLocalizer.ScroogeBank, 
...
HALLOWEEN_CHAIRMAN_REVEAL = 123
CHRISTMAS = 124
ORANGES = 125
PROLOGUE_GOOFY_CHATTER = 429774
HiddenHolidays = [HALLOWEEN_COSTUMES, HALLOWEEN_CHAIRMAN_REVEAL, SILLYMETER_HOLIDAY, SILLY_CHATTER_ONE, SILLY_CHATTER_TWO, SILLY_CHATTER_THREE, SILLY_CHATTER_FOUR, SILLY_CHATTER_FIVE, ORANGES]
SUIT_INVASION_HOLIDAYS = {SKELECOG_INVASION: 'skelecog', 
   MR_HOLLYWOOD_INVASION: 'mh', 
   MARCH_INVASION: 'bs', 
   IDES_OF_MARCH: 'bs', 
   DECEMBER_INVASION: 'cc', 
   SELLBOT_SURPRISE_1: 'cc', 
   SELLBOT_SURPRISE_2: 'nd', 
   SELLBOT_SURPRISE_3: 'tf', 
   SELLBOT_SURPRISE_4: 'mh', 
   TIGHTWAD_INVASION: 'tw', 
   CASHBOT_CONUNDRUM_1: 'tw', 
   CASHBOT_CONUNDRUM_2: 'bc', 
   CASHBOT_CONUNDRUM_3: 'ls', 
   CASHBOT_CONUNDRUM_4: 'rb', 
   LAWBOT_GAMBIT_1: 'ac', 
   LAWBOT_GAMBIT_2: 'bs', 
   LAWBOT_GAMBIT_3: 'sd', 
   LAWBOT_GAMBIT_4: 'le', 
   TROUBLE_BOSSBOTS_1: 'p', 
   TROUBLE_BOSSBOTS_2: 'ym', 
   YES_MAN_INVASION: 'ym', 
   TROUBLE_BOSSBOTS_3: 'mm', 
   MICROMANAGER_INVASION: 'mm', 
   TROUBLE_BOSSBOTS_4: 'tbc', 
   BIG_CHEESE_INVASION: 'tbc', 
   BEAN_COUNTER_INVASION: 'bc', 
   DOUBLE_TALKER_INVASION: 'dt', 
   DOUBLETALKER_INVASION: 'dt', 
   DOWN_SIZER_INVASION: 'ds', 
   DOWNSIZER_INVASION: 'ds', 
   TELEMARKETER_INVASION: 'tm', 
   HEADHUNTER_INVASION: 'hh', 
   MONEYBAGS_INVASION: 'mb', 
   MINGLER_INVASION: 'm', 
   CORPORATE_RAIDER_INVASION: 'cr', 
   BIG_WIG_INVASION: 'bw', 
   PRE_JULY_4_BIGWIG_INVASION: 'bw', 
   PRE_JULY_4_DOWNSIZER_INVASION: 'ds', 
   MOVER_AND_SHAKER_INVASION: 'ms', 
   PENNY_PINCHER_INVASION: 'pp', 
   NUMBER_CRUNCHER_INVASION: 'nc', 
   SELLBOT_INVASION: 'sellbot', 
   SELLBOT_INVASION_MOVER_AND_SHAKER: 'ms'}
...
hood2Coords = {'CEO': [
   ...
   'WW': [
        (-102.431, 6.703, 0.565),
        (-38.212, 0, 0)]}
EventManagerFreeMode = 1
EventManagerStrictMode = 2
DefaultMiniserverLimit = 16
MaxTokens = 9999
