...
class DistributedRace(DistributedObject.DistributedObject):
    ...
    def announceGenerate(self):
        ...
        musicFile = self.BGM_BaseDir + RaceGlobals.TrackDict[self.trackId][7] + '.ogg'
    ...
    def raceWatcher(self, task):
        ...
        if newLapT - self.currLapT < -0.5:
            self.lastLap = self.laps
            self.laps += 1
            self.changeMusicTempo(1 + self.laps)
    ...
    def changeMusicTempo(self, lap):
        if self.lastLap == 0 and lap == 1:
            pass
        else:
            newTime = self.raceMusic.getTime()
            if lap == 1:
                if self.lastLap == 1:
                    newTime /= 0.7143
                musicFile = self.BGM_BaseDir + RaceGlobals.TrackDict[self.trackId][7] + '.ogg'
                ...
                if lap == 2:
                    if self.lastLap == 2:
                        newTime /= 0.5
                    newTime *= 0.7143
                    musicFile = self.BGM_BaseDir + RaceGlobals.TrackDict[self.trackId][7] + '_lap2.ogg'
                    self.raceMusic.stop()
                    self.raceMusic = base.loader.loadMusic(musicFile)
                else:
                    if lap == 3:
                        newTime /= 0.7143
                        newTime *= 0.5
                        musicFile = self.BGM_BaseDir + RaceGlobals.TrackDict[self.trackId][7] + '_lap3.ogg'
                        self.raceMusic.stop()
                        self.raceMusic = base.loader.loadMusic(musicFile)
            if lap in (1, 2, 3):
                base.playMusic(self.raceMusic, looping=1, volume=0.8, time=newTime)
