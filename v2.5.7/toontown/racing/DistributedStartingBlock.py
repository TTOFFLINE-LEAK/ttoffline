...
class DistributedStartingBlock(DistributedObject.DistributedObject, FSM):
        ...
        def getToonJumpTrack(av, kart):
            ...
            if av.isDisguised:
                if av.suit.style.body == 'b':
                    ...
                    wait = 0
            av.playingAnim = av.neutralAnim
            toonJumpTrack = Parallel(Func(av.enterJump), Sequence(Wait(wait), Parallel(LerpHprInterval(av, hpr=getJumpHpr, duration=0.9), ProjectileInterval(av, endPos=getJumpDest, duration=0.9))))
            return toonJumpTrack

        def getToonSitTrack(av):
            toonSitTrack = Sequence(ActorInterval(av, 'sit-start'), Func(av.loop, 'sit'))
            if av.isDisguised:
                toonSitTrack = Sequence(Wait(2), Func(av.suit.loop, 'sit'))
            return toonSitTrack
    ...
    def generateKartDisappearTrack(self):
        ...
        if not self.holeActor:
            ...
            self.holeActor.setBlend(frameBlend=config.GetBool('interpolate-animations', True))
