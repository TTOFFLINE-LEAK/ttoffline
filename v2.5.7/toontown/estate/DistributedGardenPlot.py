...
class DistributedGardenPlot(DistributedLawnDecor.DistributedLawnDecor):
    ...
    def setMovie(self, mode, avId):
        if mode == GardenGlobals.MOVIE_PLANT:
            ...
            if mode == GardenGlobals.MOVIE_FINISHREMOVING:
                self.doFinishRemovingTrack(avId)
            else:
                if mode == GardenGlobals.MOVIE_PLANT_REJECTED:
    ...
    def setBoxDoId(self, boxId, index):
        if boxId in self.cr.doId2do:
            self.setBox(index, base.cr.doId2do[boxId])
        else:
            self.acceptOnce('generate-%d' % boxId, self.setBox, extraArgs=[index])

    def setBox(self, index, box):
