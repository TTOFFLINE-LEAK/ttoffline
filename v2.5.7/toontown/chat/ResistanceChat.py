...
RESISTANCE_MONEY = 2
RESISTANCE_TOONDOWN = 3
resistanceMenu = [RESISTANCE_TOONUP, RESISTANCE_RESTOCK, RESISTANCE_MONEY, RESISTANCE_TOONDOWN]
resistanceDict = {RESISTANCE_TOONUP: {'menuName': TTL.ResistanceToonupMenu, 'itemText': TTL.ResistanceToonupItem, 
   ...
   RESISTANCE_RESTOCK: {'menuName': TTL.ResistanceRestockMenu, 'itemText': TTL.ResistanceRestockItem, 
                        ...
                        'items': [
                                ...
                                6,
                                7]}, 
   RESISTANCE_TOONDOWN: {'menuName': TTL.ResistanceToondownMenu, 'itemText': TTL.ResistanceToondownItem, 
                         'chatText': TTL.ResistanceToondownChat, 
                         'values': [
                                  10,
                                  20,
                                  40,
...
def getItemText(textId):
    ...
    if menuIndex is RESISTANCE_TOONUP:
        ...
        if menuIndex is RESISTANCE_RESTOCK:
            ...
            if menuIndex is RESISTANCE_TOONDOWN:
                if value is -1:
                    value = TTL.ResistanceToondownItemMax
    ...
    if menuIndex == RESISTANCE_TOONUP:
        ...
        if menuIndex == RESISTANCE_MONEY:
            ...
            if menuIndex == RESISTANCE_RESTOCK:
                ...
                if menuIndex == RESISTANCE_TOONDOWN:
                    effect = BattleParticles.loadParticleFile('firedFlame.ptf')
                    fadeColor = VBase4(1, 0.5, 0, 1)
                else:
                    return
