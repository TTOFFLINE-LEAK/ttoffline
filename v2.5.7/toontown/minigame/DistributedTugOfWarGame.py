...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def initToons(self):
        for avId in self.avIdList:
            ...
            if toon:
                ...
                if toon.isDisguised:
                    toon.suit.pose('tug-o-war', 3)
                    self.rightHandDict[avId] = toon.suit.getRightHands()[0]
                else:
    ...
    def sendStopSignal(self, winners, losers, tieers):
        ...
        for avId in self.avIdList:
            ...
            if avId in winners:
                if toon.isDisguised:
                    reactSeq.append(Func(toon.suit.loop, 'victory'))
                else:
    ...
    def throwInWater(self, avId=None):
        ...
        if avId:
            if loser.isDisguised:
                self.animTracks[animId] = Sequence(Parallel(ActorInterval(actor=loser.suit, animName='slip-forward', duration=2.0), LerpPosInterval(loser, duration=2.0, pos=newPos), Sequence(Wait(1.0), Parallel(Func(base.playSfx, self.sndHitWater), Func(self.splash.play), Func(self.ripples.play)))), Func(loser.suit.loop, 'neutral'))
