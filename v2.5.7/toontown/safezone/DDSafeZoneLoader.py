...
class DDSafeZoneLoader(SafeZoneLoader.SafeZoneLoader):
    ...
    def __init__(self, hood, parentFSM, doneEvent):
        ...
        self.dnaFile = 'phase_6/dna/donalds_dock_sz.jazz'
        self.safeZoneStorageDNAFile = 'phase_6/dna/storage_DD_sz.jazz'
        self.pumpkin1 = None
    ...
    def load(self):
        ...
        if base.cr.newsManager.isHolidayRunning(ToontownGlobals.TRICK_OR_TREAT) or base.cr.newsManager.isHolidayRunning(ToontownGlobals.HALLOWEEN_PROPS) or base.cr.newsManager.isHolidayRunning(ToontownGlobals.HALLOWEEN):
            ...
            self.pumpkin1.generatePumpkin()
            self.pumpkin1.reparentTo(self.boat)
            self.pumpkin1.setPos(0.191, 14.234, 3.955)
            self.pumpkin1.loop('neutral')
            one = 1
            self.pumpkin1.setScale(one)
            appendment = self.pumpkin1.getAnim(0)
            self.pumpkinloop.append(appendment)
            self.pumpkinloop.loop()
    ...
    def unload(self):
        ...
        del self.boat
        if self.pumpkin1:
            self.pumpkin1.cleanup()
            self.pumpkin1.removeNode()
            self.pumpkinloop.finish()
