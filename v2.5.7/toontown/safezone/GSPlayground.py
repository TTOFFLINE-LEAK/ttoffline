...
class GSPlayground(Playground.Playground):
    ...
    def load(self):
        ...
        self.constructorToon = GSConstruction.GSConstruction()
        self.constructorToon.generate()
        self.sayConstructor = Sequence(Func(self.constructorToon.toonPhrase), Wait(25))
        self.sayConstructor.loop()

    def unload(self):
        Playground.Playground.unload(self)
        if self.constructorToon:
            self.constructorToon.disable()
        if self.sayConstructor:
            self.sayConstructor.finish()
