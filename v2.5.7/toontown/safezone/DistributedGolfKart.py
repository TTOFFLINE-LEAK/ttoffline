...
class DistributedGolfKart(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.kartModelPath2 = 'phase_12/models/bossbotHQ/Coggolf_cart3'
    ...
    def generate(self):
        ...
        if base.localAvatar.zoneId == ToontownGlobals.BossbotHQ:
            self.kart = loader.loadModel(self.kartModelPath2)
        else:
        ...
        def getToonJumpTrack(av, seatIndex):
            ...
            if av.isDisguised:
                ...
                if av.suit.style.body == 'a':
                    ebicJumpVariable = ActorInterval(av.suit, 'slip-forward', startFrame=55)
                else:
                    if av.suit.style.body == 'b':
                        ebicJumpVariable = Sequence(ActorInterval(av.suit, 'quick-jump', playRate=5, endFrame=15), ActorInterval(av.suit, 'quick-jump', startFrame=15, endFrame=30), ActorInterval(av.suit, 'quick-jump', startFrame=107))
                        delay = 0.1
                    else:
                        if av.suit.style.body == 'c':
                            ebicJumpVariable = ActorInterval(av.suit, 'slip-forward', startFrame=59)
                toonJumpTrack = Parallel(ebicJumpVariable, Sequence(Wait(delay), Parallel(LerpHprInterval(av, hpr=getJumpHpr, duration=0.9), ProjectileInterval(av, endPos=getJumpDest, duration=0.9))))
            ...
            if av.isDisguised:
                ...
                if av.suit.style.body == 'a':
                    ...
                    if av.suit.style.body == 'b':
                        ...
                        if av.suit.style.body == 'c':
                            ebicJumpVariable = ActorInterval(av.suit, 'slip-forward', startFrame=59)
                toonJumpTrack = Parallel(ebicJumpVariable, Sequence(Wait(delay), Parallel(ProjectileInterval(av, endPos=getJumpDest, duration=0.9))))
            return toonJumpTrack
