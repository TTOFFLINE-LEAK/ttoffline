...
class DistributedTreasure(DistributedObject.DistributedObject):
    ...
    def loadModel(self, overrideModelPath=None):
        if overrideModelPath:
            self.modelPath = overrideModelPath
        if not self.modelPath:
            self.modelPath = TreasureGlobals.TreasureModels[self.treasureType][0]
        if not self.grabSoundPath:
            self.grabSoundPath = TreasureGlobals.TreasureModels[self.treasureType][1]
