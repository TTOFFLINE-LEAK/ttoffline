...
class PartyEditor(DirectObject, FSM):
    ...
    def initElementList(self):
        ...
        for decorationId in PartyGlobals.DecorationIds:
            if not isVictory and decorationId in PartyGlobals.VictoryPartyDecorationIds or not isWinter and decorationId in PartyGlobals.WinterPartyDecorationIds or not isValentine and decorationId in PartyGlobals.ValentinePartyDecorationIds:
                ...
                self.elementList.addItem(pele)
                if decorationId == PartyGlobals.DecorationIds.NormalSky:
                    self.normalSkyElement = pele
    ...
    def initNormalSky(self):
        self.normalSkyElement.buyButtonClicked((7, 7))
    ...
    def buyCurrentElement(self):
        if self.currentElement:
            ...
            if purchaseSuccessful:
                ...
                self.handleMutuallyExclusiveDecorations()
            else:
    ...
    def handleMutuallyExclusiveDecorations(self):
        mutSet = self.getMutuallyExclusiveDecorations()
        if not mutSet:
            return
        currentDecorations = self.partyEditorGrid.getDecorationsElementsOnGrid()
        lastDecoration = self.partyEditorGrid.lastDecorationIdPlaced
        for act in currentDecorations:
            if act.id in mutSet and not lastDecoration == act.id:
                act.removeFromGrid()
                removedName = TTLocalizer.PartyDecorationNameDict[act.id]['editor']
                addedName = TTLocalizer.PartyDecorationNameDict[lastDecoration]['editor']
                instr = TTLocalizer.PartyPlannerEditorInstructionsRemoved % {'removed': removedName, 'added': addedName}
                self.partyPlanner.instructionLabel['text'] = instr
                self.updateCostsAndBank()

    def getMutuallyExclusiveDecorations(self):
        ...
        result = None
        for mutuallyExclusiveTuples in PartyGlobals.MutuallyExclusiveDecorations:
            mutSet = Set(mutuallyExclusiveTuples)
            inter = mutSet.intersection(actSet)
            if len(inter) > 1:
                result = inter
