...
class DistributedPartyActivity(DistributedObject.DistributedObject):
    ...
    def announceGenerate(self):
        ...
        self.root.setPosHpr(centeredX, centeredY, self.z, self.h, self.p, self.r)
    ...
    def load(self):
        ...
        if self.wantRewardGui:
            ...
            self.tokenGui = TokenRewardGui(self.showRewardDoneEvent)
    ...
    def showTokenReward(self, earnedAmount, jarAmount, message):
        if not self.isLocalToonInActivity() or base.localAvatar.doId in self.getToonIdsAsList():
            messenger.send('DistributedPartyActivity-showTokenReward')
            base.cr.playGame.getPlace().fsm.request('activity')
            self.acceptOnce(self.showRewardDoneEvent, self.__handleTokenRewardDone)
            self.tokenGui.showReward(earnedAmount, jarAmount, message)

    def __handleJellybeanRewardDone(self):
        self.ignore(self.showRewardDoneEvent)
        self.handleRewardDone()

    def __handleTokenRewardDone(self):
        self.ignore(self.showRewardDoneEvent)
        self.handleRewardDone()
    ...
    def unload(self):
        ...
        del self.rewardGui
        if self.tokenGui is not None:
            self.tokenGui.destroy()
        del self.tokenGui
