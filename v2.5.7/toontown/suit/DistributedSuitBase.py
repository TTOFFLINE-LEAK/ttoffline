...
class DistributedSuitBase(DistributedAvatar.DistributedAvatar, Suit.Suit, SuitBase.SuitBase):
    ...
    def setSkeleRevives(self, num):
        ...
        if self.getSkeleRevives() > 0:
            if self.dna.name == 'ols':
                nameInfo = TTLocalizer.SuitBaseNameWithoutDept % {'name': self._name, 'level': '%s%s' % (self.getActualLevel(), TTLocalizer.SkeleRevivePostFix)}
            else:
                if self.dna.name == 'ty':
                    nameInfo = TTLocalizer.SuitBaseNameWithoutDept % {'name': self._name, 'level': 'TY'}
                else:
    ...
    def setLevel(self, level):
        SuitBase.SuitBase.setLevel(self, level)

    def attachPropeller(self, sfx=True):
        if self.prop == None:
    ...
    def beginSupaFlyMove(self, pos, moveIn, trackName, walkAfterLanding=True, sfx=True):
        skyPos = Point3(pos)
        if moveIn:
    ...
    def becomeBoss(self, dna):
        Suit.Suit.makeBoss(self)
        self.dna.name = dna