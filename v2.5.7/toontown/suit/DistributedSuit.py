...
class DistributedSuit(DistributedSuitBase.DistributedSuitBase, DelayDeletable):
    ...
    def __init__(self, cr):
        ...
        self.fsm = ClassicFSM.ClassicFSM('DistributedSuit', [
         ...
         State.State('DanceThenFlyAway', self.enterDanceThenFlyAway, self.exitDanceThenFlyAway, []),
         State.State('Explode', self.enterExplode, self.exitExplode, [])], 'Off', 'Off')
        ...
        if base.textToSpeech:
            if sys.platform == 'darwin':
                if self.getTTSVolume() == 0:
                    return
                voice = '-v ' + ('Thomas' if config.GetString('language', 'english') == 'french' else 'Zarvox')
