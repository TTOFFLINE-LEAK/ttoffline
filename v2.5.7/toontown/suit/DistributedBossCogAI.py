...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def switchBoss(self, index):
        self.sendUpdate('switchBoss', [index])
...
def switchBoss(bossIndex):
    if 0 <= bossIndex <= 4:
        for boss in simbase.air.doId2do.itervalues():
            if isinstance(boss, DistributedBossCogAI):
                if boss.getLocation() == spellbook.getInvoker().getLocation():
                    boss.switchBoss(bossIndex)
                    return 'Successfully changed Boss Cog type!'

        return 'Invalid location.'
    return 'Invalid Boss Index Specified.'