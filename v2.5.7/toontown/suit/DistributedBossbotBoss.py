...
class DistributedBossbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def ceoTalk(self):
        ceoChat = ["Waiters, you'll find yourselves out of a job if my guests aren't kept fed.",
         'Get cracking and serve my banquet!',
         'Come on, we have guests to feed!',
         'Move it! Time is money!']
        self.setChatAbsolute(random.choice(ceoChat), CFSpeech | CFTimeout)
    ...
    def enterBattleTwo(self):
        ...
        self.ceoTalking = Sequence(Wait(10), Func(self.ceoTalk), Wait(24))
        self.ceoTalking.loop()
    ...
    def incrementToonLevel(self, amount):
        self.toonLevel -= amount
    ...
    def __talkAboutPromotion(self, speech):
        if self.prevCogSuitLevel < ToontownGlobals.MaxCogSuitLevel:
            newCogSuitLevel = localAvatar.getCogLevels()[CogDisguiseGlobals.dept2deptIndex('c')]
    ...
    def localToonTouchedBeltToonup(self, beltIndex, toonupIndex, toonupNum):
        self.warning = OnscreenText(text='You feel pretty full for now and decide to leave the snack alone.', pos=(0,
                                                                                                                   -0.6), scale=0.1, font=ToontownGlobals.getSuitFont(), fg=(1,
                                                                                                                                                                             1,
                                                                                                                                                                             1,
                                                                                                                                                                             1), shadow=(0.1,
                                                                                                                                                                                         0.1,
        ...
        self.warningInterval = Sequence(self.warning.colorScaleInterval(1.969, (1,
                                                                                ...
                                                                                1)), Wait(1.5), self.warning.colorScaleInterval(2, (1,
                                                                                                                                    1,
                                                                                                                                    1,
                                                                                                                                    0)), Wait(20), Func(self.incrementToonLevel, 6))
        avId = base.localAvatar.doId
        doRequest = True
        if doRequest:
            if self.toonLevel == 0:
            ...
            elif self.toonLevel == 1:
                self.toonLevel += 1
                self.sendUpdate('requestGetToonup', [beltIndex, toonupIndex, toonupNum])
            elif self.toonLevel == 2:
                self.toonLevel += 1
                self.sendUpdate('requestGetToonup', [beltIndex, toonupIndex, toonupNum])
            elif self.toonLevel == 3:
                self.toonLevel += 2
                self.sendUpdate('requestGetToonup', [beltIndex, toonupIndex, toonupNum])
            elif self.toonLevel == 5:
                self.toonLevel += 1
                self.warningInterval.start()
            elif self.toonLevel == 6:
                pass
