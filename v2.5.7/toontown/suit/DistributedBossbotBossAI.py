...
class DistributedBossbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def invokeSuitPlanner(self, buildingCode, skelecog):
        ...
        for activeSuit in activeSuits:
            activeSuit.b_setSkeleRevives(1)

        reserveSuits = suits['reserveSuits'][:]
        for reserveSuit in reserveSuits:
            reserveSuit[0].b_setSkeleRevives(1)
    ...
    def setupBattleFourObjects(self):
        ...
        if not self.newSuit:
            ...
            self.newSuit = random.choice(possibleNewSuits)
            self.updateSuit(self.newSuit)
        self.battleFourSetup = True

    def updateSuit(self, newSuit):
        self.sendUpdate('setNewSuit', [newSuit])
