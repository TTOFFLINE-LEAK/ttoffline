...
import DistributedSquirtingFlowerSuitAI, DistributedSuitAI, SuitDNA, SuitPlannerBase, SuitTimings
...
from toontown.dna.SuitLeg import SuitLeg
...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    def initBuildingsAndPoints(self):
        ...
        for p in self.frontdoorPointList:
            ...
            if p < 0 and not isinstance(p, DNASuitPoint):
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover=None, cogdoTakeover=None, minPathLen=None, maxPathLen=None, buildingHeight=None, suitLevel=None, suitType=None, suitTrack=None, suitName=None, specialSuit=0):
        ...
        if startPoint == None:
            return
        if self.getZoneId() == ToontownGlobals.OldOakStreet and self.air.avIsInEpisode() and self.air.currentEpisode == 'squirting_flower':
            newSuit = DistributedSquirtingFlowerSuitAI.DistributedSquirtingFlowerSuitAI(simbase.air, self)
        else:
        ...
        if specialSuit == 1 or suitTrack == 's' and suitType == 9:
            if specialSuit == 0 and suitName in SuitDNA.extraSuits:
                pass
            else:
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            self.notify.debug('Existing buildings are (%s, %s), choosing from (%s, %s), chose %s, %s.' % (
            ...
            if buildingTrack != None and buildingHeight != None:
                ...
                self.notify.debug('Assigning building to zone %d, pending tracks = %s, pending heights = %s' % (
    ...
    def requestBattle(self, zoneId, suit, toonId):
        ...
        if config.GetBool('props-buff-battles', True) and self.cellToGagBonusDict.has_key(canonicalZoneId):
    ...
    def pickLevelTypeAndTrack(self, level=None, type=None, track=None):
        ...
        if type == None:
            ...
            if type == 9 and track == 'g':
                type = 8
                level = min(max(level, type), type + 4)
            else:
                if type == 9 and track != 'm':
                    level = 50
                else:
                    if type == 8:
...
@magicWord(types=[str, int, int, str], category=CATEGORY_OVERRIDE)
def spawnCog(name, level, specialSuit=0, password='whatISApassWORD'):
    av = spellbook.getInvoker()
    zoneId = av.getLocation()[1]
    if zoneId - zoneId % 100 == ToontownGlobals.OldOakStreet or zoneId in [ToontownGlobals.BossbotHQ]:
        return 'Unable to spawn %s in current zone.' % name
    if name in SuitDNA.rank9Cogs:
        return ('{0} can only be spawned with a Swagtory summon.').format(name)
    if name in SuitDNA.extraSuits:
        if name in SuitDNA.spawnableExtrasSpecial:
            return 'This Cog currently cannot be summoned.'
        if name in SuitDNA.spawnableExtras and name not in SuitDNA.spawnableExtrasWithPass:
            pass
        elif name in SuitDNA.spawnableExtras and name in SuitDNA.spawnableExtrasWithPass and (pbkdf2_sha512.verify(password, MagicWordToPasswordHash.get('spawnCog')[0]) and name == 'sm' or pbkdf2_sha512.verify(password, MagicWordToPasswordHash.get('spawnCog')[1]) and name == 'cag'):
            pass
        else:
            return 'This Cog currently cannot be summoned.'
