...
ExclusiveModel = 'phase_14/models/char/bossCog'
AprilFoolsModel = 'phase_14/models/char/aprilBoss'
LawbotStatueModel = 'phase_14/models/char/lawbotStatueBoss'
ModelDict = {'s': 'phase_9/models/char/sellbotBoss', 'm': 'phase_10/models/char/cashbotBoss', 
   ...
   'g': 'phase_14/models/char/chairbotBoss'}
DummyModelDict = {'s': 'phase_14/models/char/sellbotBoss', 'm': 'phase_14/models/char/cashbotBoss', 
   'l': 'phase_14/models/char/lawbotBoss', 
   'c': 'phase_14/models/char/bossbotBoss', 
   'g': 'phase_14/models/char/chairbotBoss'}
AnimList = ('Ff_speech', 'ltTurn2Wave', 'wave', 'Ff_lookRt', 'turn2Fb', 'Ff_neutral',
            ...
            'golf_swing', 'point')
ExclusiveAnimList = ['death', 'Ff_lookLt', 'Ff_point', 'Bb_point', 'charge', 'canAttack', 'Fb_soak', 'Fb_squirt-small', 'Fb_anvil-drop', 'Fb_safe-throw']
...
class BossCog(Avatar.Avatar):
    ...
    def generateBossCog(self):
        ...
        self.dialogArray = [self.grunt,
         ...
         self.statement]
        dna = self.style
        dummyBossFilePrefix = DummyModelDict[dna.dept]
        ...
        if base.cr.newsManager.isHolidayRunning(ToontownGlobals.APRIL_FOOLS_COSTUMES):
            if dna.dept == 'l':
                self.loadModel(LawbotStatueModel + '-torso-zero', 'torso')
                self.loadModel(LawbotStatueModel + '-head-zero', 'head')
            else:
                self.loadModel(AprilFoolsModel + '-torso-zero', 'torso')
                self.loadModel(filePrefix + '-head-zero', 'head')
            if dna.dept == 'c':
                self.find('**/__Actor_head').hide()
        else:
            if self.dummyBoss and dna.dept != 'g':
                self.loadModel(filePrefix + '-torso-zero', 'torso')
                self.loadModel(dummyBossFilePrefix + '-head-ruined', 'head')
            else:
                self.loadModel(filePrefix + '-torso-zero', 'torso')
                self.loadModel(filePrefix + '-head-zero', 'head')
        ...
        for partName in ('legs', 'torso', 'head'):
            ...
            for anim in ExclusiveAnimList:
                animDict[anim] = '%s-%s-%s' % (ExclusiveModel, partName, anim)
    ...
    def __doGetAnimIval(self, anim, raised, forward, happy):
        ...
        if startNeckHpr != endNeckHpr:
            ...
            if endNeckHpr != self.neckForwardHpr:
    ...
    def generateCorporateTie(self, modelPath=None):
        ...
        tie = modelPath.find('**/bossCog_tie')
        if tie.isEmpty():
            self.notify.warning('boss has no tie model!')
            return
        if dept == 'c':
