...
class DistributedSuitBaseAI(DistributedAvatarAI.DistributedAvatarAI, SuitBase.SuitBase):
    ...
    def setLevel(self, lvl=None):
        ...
        try:
            ...
            self.notify.warning(("We're going down! Tried to spawn a {0} level {1}").format(self.dna.name, self.level))
            raise IndexError('An issue with Extra Cogs has occured!  DNA Name: %s, Level: %d' % (self.dna.name, self.level))
