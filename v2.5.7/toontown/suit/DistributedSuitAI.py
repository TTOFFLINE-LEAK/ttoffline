...
class DistributedSuitAI(DistributedSuitBaseAI.DistributedSuitBaseAI):
    ...
    def requestBattle(self, x, y, z, h, p, r):
        ...
        if self.pathState == 3:
            ...
            if self.pathState != 1:
                ...
                if self.pathState == 2 or self.pathState == 4 or self.pathState == 5:
    ...
    def blowUp(self):
        if self.isDoomed:
            return
        self.isDoomed = True
        self.b_setPathState(5)
        self.stopPathNow()
        name = self.taskName('blowUp')
        taskMgr.remove(name)
        taskMgr.doMethodLater(SuitTimings.suitDeath, self.finishFlyAwayNow, name)
    ...
    def startTakeOver(self):
        ...
        if dept == 'g':
            return
        if self.dna.name in SuitDNA.rank9Cogs:
            return
        if self.dna.name in SuitDNA.spawnableExtras:
            return
        blockNumber = self.buildingDestination
    ...
    def becomeBoss(self):
        self.isBoss = 1
        dnaName = 'pm' + self.dna.name
        self.dna.name = dnaName
        self.sendUpdate('becomeBoss', [dnaName])